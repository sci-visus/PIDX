...
int test_multi_patch_writer(struct Args args, int rank, int nprocs) 
{
  ...
  for(var = 0; var < args.variable_count; var++)
  {
    ...
    {
      PIDX_set_point_5D(local_offset_point[var][p], (int64)var_offset[var][p][0], (int64)var_offset[var][p][1], (int64)var_offset[var][p][2], 0, 0);
      PIDX_set_point_5D(local_box_count_point[var][p], (int64)var_count[var][p][0], (int64)var_count[var][p][1], (int64)var_count[var][p][2], 1, 1);       
    }
  }
  ...
  for (ts = 0; ts < args.time_step; ts++) 
  {
	...
	for (v = 0; v < var_count[var][p][4]; v++)
	  for (u = 0; u < var_count[var][p][3]; u++)
		...
		{
		  int64 index = (int64) (var_count[var][p][0] * var_count[var][p][1] * var_count[var][p][2] * var_count[var][p][3] * v) + 
