...
int main(int argc, char **argv) 
{
    ...
    MPI_Bcast(&aggregation, 1, MPI_INT, 0, MPI_COMM_WORLD);
...
#if 1
//     if(rank >= 32)
    if(offset_local[0]+count_local[0] == gextent[0] || offset_local[1]+count_local[1] == gextent[1] || offset_local[2]+count_local[2] == gextent[2])
    {
		...
		for (u = 0; u < count[3]; u++)
			...
			for (j = 0; j < count[1]; j++)
				...
				long long index = (long long) (count[0] * count[1] * count[2] * count[3] * v) + (count[0] * count[1] * count[2] * u) + (count[0] * count[1] * k) + (count[0] * j) + i;
				    ...
				    write_data[vc][index * sample_per_variable_buffer[vc] + spv] = rank;//100 + spv + /*vc + ts +*/ ((extents[0] * extents[1] * extents[2] * extents[3] * (offset[4] + v)) + (extents[0] * extents[1] * extents[2] * (offset[3] + u)) + (extents[0] * extents[1]*(offset[2] + k))+(extents[0]*(offset[1] + j)) + (offset[0] + i));
			    }
        }
        
        idx_ptr = PIDX_create(MPI_COMM_WORLD, ts, output_file, bits_per_block, /*bpf*/256, total_dimensions, /*aggf*/1, gextent, aggregation, 1);
	PIDX_set_resolution(idx_ptr, resolution);
        for (vc = 0; vc < number_of_variables; vc++) 
	{
	    ...
	    PIDX_variable_local_add(idx_ptr, variable_ptr[vc], (int*) offset, (int*) count);
            PIDX_variable_local_layout(idx_ptr, variable_ptr[vc], write_data[vc], MPI_DOUBLE);
        }
...
static void usage(void) 
{
    printf("Usage: sparse-data-writer -g 4x4x4 -l 2x2x2 -f Filename_ -t 4 -a 1 -r 0\n");
