...
static double time_start = 0.0, time_delta = 0.0;
...
void flipEndianness(double *buffer, int count)
{
    int i, j;
    for (i = 0; i < count; ++i) {
        char *bytep = (char*)(buffer + i);
        for (j = 0; j < 4; ++j) {
            char tmp = bytep[j];
            bytep[j] = bytep[7 - j];
            bytep[7 - j] = tmp;
...
void fixFormat(double **buffer, int xdim, int ydim, int zdim, int varlen, double tag)
{
    int nsamples = xdim * ydim * zdim;

    double *xp = *buffer;
    double *yp = xp + nsamples;
    double *zp = yp + nsamples;

    double *nbuff = (double*)malloc(nsamples * varlen * sizeof(double));

    int i;

    flipEndianness(*buffer, nsamples * varlen);
#if 1
    for (i = 0; i < nsamples; ++i) {
#if 1
        nbuff[i*varlen + 0] = xp[i];
        nbuff[i*varlen + 1] = yp[i];
        nbuff[i*varlen + 2] = zp[i];
#else
        nbuff[i*varlen + 0] = (double)(i)/(double)(nsamples);
        nbuff[i*varlen + 1] = 0.5;
        nbuff[i*varlen + 2] = tag;
#endif
    }
...
#if 0
void dump_doubles(const double *buff, int xdim, int ydim, int zdim, const char *fname)
{
    FILE *fp;
    fp = fopen(fname, "w");
    int nsamples = xdim * ydim * zdim, i;
    for (i = 0; i < nsamples; ++i) {
...
int main(int argc, char **argv) {
    ...
    sample_per_variable[0] = 3; //Variable 1
    ...
    MPI_Bcast(&time_start, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    MPI_Bcast(&pidx_file_template, 512, MPI_CHAR, 0, MPI_COMM_WORLD);
    ...
    for (i = 0; i < time_step; i++) {
        //double tval = time_start + ((double)(i) * 0.002);
        fortran_file_name[i] = (char*) malloc(sizeof (char) * 512);
        //sprintf(fortran_file_name[i], "%s%05d", fortran_file_template, i);
        //sprintf(fortran_file_name[i], "%s%.4fE-03", fortran_file_template, tval);
        sprintf(fortran_file_name[i], "%s", fortran_file_template);
    }
...
#else
    pidx_offset[2] = (rank / (sub_div[0] * sub_div[1]));
    pidx_offset[0] = (rank-(pidx_offset[2]*sub_div[0]*sub_div[1])) % sub_div[0];
    pidx_offset[1] = (rank-(pidx_offset[2]*sub_div[0]*sub_div[1])) / sub_div[0];
#endif
...
#else
      fortran_offset[i][2] = (i / (sub_div[0] * sub_div[1]));
      fortran_offset[i][0] = (i-(fortran_offset[i][2]*sub_div[0]*sub_div[1])) % sub_div[0];
      fortran_offset[i][1] = (i-(fortran_offset[i][2]*sub_div[0]*sub_div[1])) / sub_div[0];
#endif
    ...
    PIDX_time_step_define(0, 9999, "time%04d/");

    for(t = 0 ; t < time_step ; t++)
    {
        double cur_tstep = 0.0;
        sscanf(basename(fortran_file_name[t]), "d-%lf", &cur_tstep);
        cur_tstep *= 1000;

        int time_index = (int)(((cur_tstep - time_start)/time_delta) + 0.5);
        //printf("cur_tstep: %f, index: %d ((%f-%f)/%f)\n", cur_tstep, time_index,
        //       cur_tstep, time_start, time_delta);

	for(i = 0 ; i < (fortran_core_count/pidx_core_count) ; i++)
	{
	    off_t data_offset = 8294476; // start of u(3)
	    sprintf(filename, "%s%sfield.%05d", fortran_file_name[t], "/", fortran_pidx_index[i]);
	    ...
	    double tag = (double)(fortran_pidx_index[i])/(double)(pidx_core_count);
#if 1
        ...
        fixFormat(fortran_buffer[i] + var, fortran_count[0], fortran_count[1], fortran_count[2], sample_per_variable[var], tag);
        //if (fortran_pidx_index[i] == 0) {
        //    char dfname[512];
        //    sprintf(dfname, "%s%sdump.%05d", fortran_file_name[0], "/", fortran_pidx_index[i]);
        //    dump_doubles(fortran_buffer[i][var], fortran_count[0], fortran_count[1], fortran_count[2], dfname);
        //}
        ...
        //printf("read %d bytes starting from %ld\n", ret, data_offset);
        ...
        for(var = 0 ; var < number_of_variables ; var++) {
            ...
            int nsamples = fortran_count[0] * fortran_count[1] *
                           fortran_count[2];
            ret = fread(fortran_buffer[i][var], sizeof(double), nsamples * 3,
                        fp);
            assert(ret == nsamples * 3);

            fixFormat(fortran_buffer[i] + var, fortran_count[0],
                      fortran_count[1], fortran_count[2], tag);

            if (fortran_pidx_index[i] == 0) {
                char dfname[512];
                sprintf(dfname, "%s%sdump.%05d", fortran_file_name[0], "/",
                        fortran_pidx_index[i]);
                dump_doubles(fortran_buffer[i][var], fortran_count[0],
                             fortran_count[1], fortran_count[2], dfname);
            }

            data_offset += nsamples * 3;
        }
	...
	idx_ptr = PIDX_create(MPI_COMM_WORLD, time_index, pidx_file_name, 15/*bits_per_block*/, 256 /*blocks_per_file*/, 3 /*3D Data*/, 1/*aggregation factor*/, gextent, 1, 0);
	PIDX_set_resolution(idx_ptr, 0);
        for (var = 0; var < number_of_variables; var++) {
            sprintf(var_name, "velocity");
    ...
    while ((one_opt = getopt(argc, argv, flags)) != EOF) {
        ...
        switch (one_opt) {
	        ...
	        case('o'):
                ...
                break;
            case('s'):
                sscanf(optarg, "%lf", &time_start);
                break;
            case('d'):
                sscanf(optarg, "%lf", &time_delta);
                break;
            case('?'):
                return (-1);
        }
...
static void usage(void) {
    ...
    printf("  -f: fortran (per-process) dimensions of dataset\n");
    printf("  -p: PIDX (per-process) dimensions of dataset\n");
    printf("  -i: input time-step folder name\n");
    printf("  -o: idx file name\n");
    printf("  -s: starting time step value\n");
    printf("  -d: time step delta value\n");
    printf("\n");
    return;
}
