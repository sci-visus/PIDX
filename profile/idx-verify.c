...
static void Hz_to_xyz(const char* bitmask,  int maxh, int64 hzaddress, int64* xyz);
static int RegExBitmaskBit(const char* bitmask_pattern,int N);
static uint64 getPowerOf2(int x);
...
{  
  ...
  max_files = (getPowerOf2(global_bounds[0]) * getPowerOf2(global_bounds[1]) * getPowerOf2(global_bounds[2]) * getPowerOf2(global_bounds[3]) * getPowerOf2(global_bounds[4])) / ((int64) pow(2, bits_per_block) * (int64) blocks_per_file);
  if ((getPowerOf2(global_bounds[0]) * getPowerOf2(global_bounds[1]) * getPowerOf2(global_bounds[2]) * getPowerOf2(global_bounds[3]) * getPowerOf2(global_bounds[4])) % ((int64) pow(2, bits_per_block) * (int64) blocks_per_file))
    max_files++;
  assert(max_files != 0);
  ...
  {
    int64 lost_element_count = 0, element_count = 0;
    for (i = 0; i < max_files; i++) 
    {
      if (existing_file_index[i] == 1) 
      {
        ...
        int bpf = 0;
        uint64* long_long_buffer = NULL;
        double* double_buffer = NULL;
        int check_bit = 1, s = 0;
        int64 hz_index, hz_val;
        ...
        for (var = 0; var < variable_count; var++) 
        {
          for (bpf = 0; bpf < blocks_per_file; bpf++) 
          {
            if (is_block_present((bpf + (blocks_per_file * i)), global_block_layout)) 
            {
              ...
              for (hz_val = 0; hz_val < data_size/(sizeof(uint64) * values_per_sample[var]); hz_val++) 
              {
                ...
                if (check_bit == 0)
                {
                  ...
                  //       var,
                  //       lost_element_count, bpf, (int64)hz_index/samples_per_block, hz_val, 
                  //       hz_index, ZYX[0], ZYX[1], ZYX[2], 
                  //       (uint64)long_long_buffer[hz_val * values_per_sample[var] + 0], (100 + var + (global_bounds[0] * global_bounds[1] * ZYX[2])+(global_bounds[0]*(ZYX[1])) + ZYX[0]));
                }
                else 
                {
                  element_count++;
    ...
    printf("[=]%lld + [!=]%lld (%lld) : %lld\n", (int64) (element_count), (int64)lost_element_count, element_count + lost_element_count, (int64) global_bounds[0] * global_bounds[1] * global_bounds[2] * global_bounds[3] * global_bounds[4] * variable_count);
    assert(element_count == (int64) global_bounds[0] * global_bounds[1] * global_bounds[2] * global_bounds[3] * global_bounds[4] * variable_count);
...
static void Hz_to_xyz(const char* bitmask,  int maxh, int64 hzaddress, int64* xyz)
{
  int64 lastbitmask=((int64)1)<<maxh;
