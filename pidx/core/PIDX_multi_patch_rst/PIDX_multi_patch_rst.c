...
int contains_patch(Ndim_patch reg_patch, Ndim_patch* patches, int count)
{
  int i=0;
...
//      }
   ...
   for(pc0 = 0; pc0 < rst_id->idx->variable[start_var_index]->sim_patch_count; pc0++)
    ...
    //printf("%d FOUND reg_multi_patch_grp_count %d\n", rank, rst_id->reg_multi_patch_grp_count);
        ...
        for (k = 0; k < adjusted_bounds[2]; k = k + rst_id->reg_patch_size[2])
          for (l = 0; l < adjusted_bounds[3]; l = l + rst_id->reg_patch_size[3])
            for (m = 0; m < adjusted_bounds[4]; m = m + rst_id->reg_patch_size[4])
            {
              ...
              if (intersectNDChunk(reg_patch, local_proc_patch) && !contains_patch(reg_patch, found_reg_patches, found_reg_patches_count))//&& intersected == 0)
              {
                found_reg_patches[found_reg_patches_count] = (Ndim_patch)malloc(sizeof (*reg_patch));
                memcpy(found_reg_patches[found_reg_patches_count], reg_patch, sizeof (*reg_patch));
                found_reg_patches_count++;
...
//                      curr_patch->size[d] = rst_id->idx_derived->rank_r_count[PIDX_MAX_DIMENSIONS * r + d];
                ...
                //printf("rank %d max_patch_rank %d count %d\n", rank, patch_grp->max_patch_rank, patch_grp->count);
...
PIDX_return_code PIDX_multi_patch_rst_staged_write(PIDX_multi_patch_rst_id rst_id)
{
  ...
  for (start_index = rst_id->first_index; start_index < (rst_id->last_index + 1); start_index = start_index + pipe_length + 1)
  {
    ...
    //if (rank == 0)
    //  printf("Reached Line %d: %d [%d %d %d] -- %d\n", __LINE__, sizeof (*chunk_data_type) * req_count  * (end_index - start_index + 1), sizeof (*chunk_data_type), req_count, (end_index - start_index + 1), rst_id->reg_multi_patch_grp_count);
        ...
        for(j = 0; j < rst_id->reg_multi_patch_grp[i]->count; j++)
        {
          ...
          else
          {
            for(v = start_index; v <= end_index; v++)
            {
              ...
              //ret = MPI_Recv(var->rst_patch_group[counter]->patch[j]->buffer, length, MPI_BYTE, rst_id->reg_multi_patch_grp[i]->source_patch_rank[j], 123, rst_id->comm, status);
              if (ret != MPI_SUCCESS)
              {
                fprintf(stderr, "Error: File [%s] Line [%d]\n", __FILE__, __LINE__);
                return PIDX_err_mpi;
        ...
        for(j = 0; j < rst_id->reg_multi_patch_grp[i]->count; j++)
        {
          if(rank == rst_id->reg_multi_patch_grp[i]->source_patch[j].rank)
          {
            for(v = start_index; v <= end_index; v++)
            {
              ...
              //ret = MPI_Send(var->sim_patch[0]->buffer, 1, chunk_data_type, rst_id->reg_multi_patch_grp[i]->max_patch_rank, 123, rst_id->comm);
              if (ret != MPI_SUCCESS)
              {
                fprintf(stderr, "Error: File [%s] Line [%d]\n", __FILE__, __LINE__);
                return PIDX_err_mpi;
            ...
            for (b1 = reg_patch_offset[3]; b1 < reg_patch_offset[3] + reg_patch_count[3]; b1++)
              for (k1 = reg_patch_offset[2]; k1 < reg_patch_offset[2] + reg_patch_count[2]; k1++)
                for (j1 = reg_patch_offset[1]; j1 < reg_patch_offset[1] + reg_patch_count[1]; j1++)
                  for (i1 = reg_patch_offset[0]; i1 < reg_patch_offset[0] + reg_patch_count[0]; i1 = i1 + reg_patch_count[0])
                  {
                    // unsigned long long *sim_patch_offset = rst_id->idx->variable[rst_id->first_index]->sim_patch[0]->offset;
                    // unsigned long long *sim_patch_count = rst_id->idx->variable[rst_id->first_index]->sim_patch[0]->size;
      ...
      for(j = 0; j < rst_id->reg_multi_patch_grp[i]->count; j++)
      {
        if(rank == rst_id->reg_multi_patch_grp[i]->source_patch[j].rank)
        {
          for(v = rst_id->first_index; v <= rst_id->last_index; v++)
          {
            ...
            printf("%d: p %d v %d count %lld %lld %lld %lld %lld off %lld %lld %lld %lld %lld\n", rank, j, v, reg_patch_count[0],reg_patch_count[1],reg_patch_count[2],reg_patch_count[3],reg_patch_count[4],
              reg_patch_offset[0],reg_patch_offset[1],reg_patch_offset[2],reg_patch_offset[3],reg_patch_offset[4]);
...
#if !SIMULATE_IO
            ret = MPI_Irecv(var->sim_patch[p_index]->buffer, 1, chunk_data_type[chunk_counter], rst_id->reg_multi_patch_grp[i]->max_patch_rank, 123, rst_id->comm, &req[req_counter]);
            
            //ret = MPI_Irecv(var->sim_patch[0]->buffer, 1, chunk_data_type[chunk_counter], rst_id->reg_multi_patch_grp[i]->max_patch_rank, 123, rst_id->comm, &req[req_counter]);
            if (ret != MPI_SUCCESS)
            {
              fprintf(stderr, "Error: File [%s] Line [%d]\n", __FILE__, __LINE__);
              return PIDX_err_mpi;
