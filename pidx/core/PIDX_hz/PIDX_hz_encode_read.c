        ...
        for (j = chunked_patch_offset[1]; j < chunked_patch_offset[1] + chunked_patch_size[1]; j++)
          for (i = chunked_patch_offset[0]; i < chunked_patch_offset[0] + chunked_patch_size[0]; i++)
          {
            ...
            for(v1 = id->first_index; v1 <= id->last_index; v1++)
            {
              ...
              //float x;
              //memcpy(&x, var_grp->variable[v1]->hz_buffer->buffer[level] + (hz_index * bytes_for_datatype), sizeof(float));
              //fprintf(stderr, "%f\n", x);
              memcpy(var_grp->variable[v1]->chunk_patch_group->patch[b]->buffer + (index * bytes_for_datatype), var_grp->variable[v1]->hz_buffer->buffer[level] + (hz_index * bytes_for_datatype), bytes_for_datatype);
            }
          }
    }
    else
        ...
        for (j = chunked_patch_offset[1]; j < chunked_patch_offset[1] + chunked_patch_size[1]; j++)
          for (i = chunked_patch_offset[0]; i < chunked_patch_offset[0] + chunked_patch_size[0]; i++)
          {
            ...
            for(v1 = id->first_index; v1 <= id->last_index; v1++)
            {
              ...
              {
                memcpy(var_grp->variable[v1]->chunk_patch_group->patch[b]->buffer + ((index * var_grp->variable[v1]->vps) + s) * bytes_for_datatype * chunk_size, var_grp->variable[v1]->hz_buffer->buffer[level] + ((hz_index * var_grp->variable[v1]->vps + s) * bytes_for_datatype * chunk_size), bytes_for_datatype * chunk_size);
              }
            }
          }
    }
