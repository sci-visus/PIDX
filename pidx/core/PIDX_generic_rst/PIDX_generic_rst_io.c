...
PIDX_return_code PIDX_generic_rst_forced_raw_read(PIDX_generic_rst_id generic_rst_id)
{
  int temp_max_dim = 3;
  PIDX_variable_group var_grp = generic_rst_id->idx->variable_grp[generic_rst_id->group_index];
  int svi = generic_rst_id->first_index;
  int evi = generic_rst_id->last_index;
  ...
  strncpy(idx_directory_path, generic_rst_id->idx->filename, strlen(generic_rst_id->idx->filename) - 4);
  ...
  for (pc1 = 0; pc1 < var_grp->variable[svi]->sim_patch_count; pc1++)
  {
    ...
    for (n = 0; n < number_cores; n++)
    {
      ...
      for (m = 0; m < pc; m++)
      {
        ...
        {
          //sprintf(file_name, "%s/time%09d/%d_%d", directory_path, generic_rst_id->idx->current_time_step, n, m);
    ...
    {
      sprintf(file_name, "%s/time%09d/%d_%d", directory_path, generic_rst_id->idx->current_time_step, patch_grp->source_patch_rank[i], source_patch_id[i]);
