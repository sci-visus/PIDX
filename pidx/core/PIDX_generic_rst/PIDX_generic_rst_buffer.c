...
PIDX_return_code PIDX_generic_rst_buf_create(PIDX_generic_rst_id generic_rst_id)
{
  ...
  if(generic_rst_id->idx->enable_rst == 1)
  {
    int cnt = 0, i = 0;
...
PIDX_return_code PIDX_generic_rst_aggregate_buf_create(PIDX_generic_rst_id generic_rst_id)
{
  ...
  for (v = generic_rst_id->first_index; v <= generic_rst_id->last_index; ++v)
  {
    ...
    for (g = 0; g < var->patch_group_count; ++g)
    {
      ...
      var->rst_patch_group[g]->reg_patch->buffer = malloc((uint32_t)(nx * ny * nz * (var->bpv/8) * var->vps));
      memset(var->rst_patch_group[g]->reg_patch->buffer, 0,(uint32_t)(nx * ny * nz * (var->bpv/8) * var->vps));
      ...
      for (k1 = patch_group0->patch[r]->offset[2]; k1 < patch_group0->patch[r]->offset[2] + patch_group0->patch[r]->size[2]; k1++)
      {
        for (j1 = patch_group0->patch[r]->offset[1]; j1 < patch_group0->patch[r]->offset[1] + patch_group0->patch[r]->size[1]; j1++)
        {
          for (i1 = patch_group0->patch[r]->offset[0]; i1 < patch_group0->patch[r]->offset[0] + patch_group0->patch[r]->size[0]; i1 = i1 + patch_group0->patch[r]->size[0])
          {
            ...
            for (v = generic_rst_id->figeneric_rst_index; v <= rst_id->last_index; ++v)
            {
              PIDX_variable var = var_grp->variable[v];
              Ndim_patch out_patch = var->rst_patch_group[g]->reg_patch;
