...
PIDX_return_code PIDX_wavelet_rst_buf_aggregate_and_write(PIDX_wavelet_rst_id wavelet_rst_id)
{
  int v;
  char *directory_path;
  directory_path = malloc(sizeof(*directory_path) * PATH_MAX);
  memset(directory_path, 0, sizeof(*directory_path) * PATH_MAX);
  strncpy(directory_path, wavelet_rst_id->idx->filename, strlen(wavelet_rst_id->idx->filename) - 4);

  int g = 0;
  PIDX_variable_group var_grp = wavelet_rst_id->idx->variable_grp[wavelet_rst_id->group_index];
  PIDX_variable var0 = var_grp->variable[wavelet_rst_id->first_index];
  for (g = 0; g < var0->patch_group_count; ++g)
  {
    ...
    sprintf(file_name, "%s/time%09d/%d_%d", directory_path, wavelet_rst_id->idx->current_time_step, wavelet_rst_id->idx_c->grank, g);
    int fp = open(file_name, O_CREAT | O_WRONLY, 0664);

    int v_start = 0, v_end = 0;
    int svi = wavelet_rst_id->first_index;
    int evi = wavelet_rst_id->last_index + 1;
    for (v_start = svi; v_start < evi; v_start = v_start + (wavelet_rst_id->idx_d->raw_io_pipe_length + 1))
    {
      v_end = ((v_start + wavelet_rst_id->idx_d->raw_io_pipe_length) >= (evi)) ? (evi - 1) : (v_start + wavelet_rst_id->idx_d->raw_io_pipe_length);
...
PIDX_return_code PIDX_wavelet_rst_buf_read_and_aggregate(PIDX_wavelet_rst_id wavelet_rst_id)
{
  int v;
  MPI_File fh;
  ...
  strncpy(directory_path, wavelet_rst_id->idx->filename, strlen(wavelet_rst_id->idx->filename) - 4);
  sprintf(data_set_path, "%s/time%09d/", directory_path, wavelet_rst_id->idx->current_time_step);

  for (v = wavelet_rst_id->first_index; v <= wavelet_rst_id->last_index; ++v)
  {
    PIDX_variable var = var_grp->variable[v];
...
PIDX_return_code PIDX_wavelet_rst_buf_aggregated_write(PIDX_wavelet_rst_id wavelet_rst_id)
{
  int g = 0;
  char *directory_path;
  directory_path = malloc(sizeof(*directory_path) * PATH_MAX);
  ...
  for (g = 0; g < var0->patch_group_count; ++g)
  {
    ...
    for (v_start = svi; v_start < evi; v_start = v_start + 1)
    {
      ...
      if (wavelet_rst_id->idx->flip_endian == 1)
      {
...
PIDX_return_code PIDX_wavelet_rst_buf_aggregated_read(PIDX_wavelet_rst_id wavelet_rst_id)
{
  int g = 0;
  char *directory_path;
  directory_path = malloc(sizeof(*directory_path) * PATH_MAX);
...
PIDX_return_code PIDX_wavelet_rst_forced_raw_read(PIDX_wavelet_rst_id wavelet_rst_id)
{
  int temp_max_dim = 3;
  PIDX_variable_group var_grp = wavelet_rst_id->idx->variable_grp[wavelet_rst_id->group_index];
  int svi = wavelet_rst_id->first_index;
  int evi = wavelet_rst_id->last_index;
  ...
  strncpy(idx_directory_path, wavelet_rst_id->idx->filename, strlen(wavelet_rst_id->idx->filename) - 4);
  ...
  for (pc1 = 0; pc1 < var_grp->variable[svi]->sim_patch_count; pc1++)
  {
    ...
    for (n = 0; n < number_cores; n++)
    {
      ...
      for (m = 0; m < pc; m++)
      {
        ...
        {
          //sprintf(file_name, "%s/time%09d/%d_%d", directory_path, wavelet_rst_id->idx->current_time_step, n, m);
    ...
    {
      sprintf(file_name, "%s/time%09d/%d_%d", directory_path, wavelet_rst_id->idx->current_time_step, patch_grp->source_patch_rank[i], source_patch_id[i]);
