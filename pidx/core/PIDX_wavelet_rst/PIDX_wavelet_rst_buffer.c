...
PIDX_return_code PIDX_wavelet_rst_buf_create(PIDX_wavelet_rst_id wavelet_rst_id)
{
  int j = 0, v = 0, p = 0;
  PIDX_variable_group var_grp = wavelet_rst_id->idx->variable_grp[wavelet_rst_id->group_index];

  if(wavelet_rst_id->idx->enable_rst == 1)
  {
    int cnt = 0, i = 0;
    for (v = wavelet_rst_id->first_index; v <= wavelet_rst_id->last_index; v++)
    {
      ...
      cnt = 0;
      for (i = 0; i < wavelet_rst_id->reg_patch_grp_count; i++)
      {
...
PIDX_return_code PIDX_wavelet_rst_buf_destroy(PIDX_wavelet_rst_id wavelet_rst_id)
{
  int i, j, v;
...
PIDX_return_code PIDX_wavelet_rst_aggregate_buf_create(PIDX_wavelet_rst_id wavelet_rst_id)
{
  int v = 0;
...
PIDX_return_code PIDX_wavelet_rst_aggregate_buf_destroy(PIDX_wavelet_rst_id wavelet_rst_id)
{
  PIDX_variable_group var_grp = wavelet_rst_id->idx->variable_grp[wavelet_rst_id->group_index];
  int v = 0;
  for (v = wavelet_rst_id->first_index; v <= wavelet_rst_id->last_index; ++v)
...
PIDX_return_code PIDX_wavelet_rst_buf_aggregate(PIDX_wavelet_rst_id wavelet_rst_id, int mode)
{
#if 1
  int v = 0;
  PIDX_variable_group var_grp = wavelet_rst_id->idx->variable_grp[wavelet_rst_id->group_index];
      ...
      for (k1 = patch_group0->patch[r]->offset[2]; k1 < patch_group0->patch[r]->offset[2] + patch_group0->patch[r]->size[2]; k1++)
      {
        for (j1 = patch_group0->patch[r]->offset[1]; j1 < patch_group0->patch[r]->offset[1] + patch_group0->patch[r]->size[1]; j1++)
        {
          for (i1 = patch_group0->patch[r]->offset[0]; i1 < patch_group0->patch[r]->offset[0] + patch_group0->patch[r]->size[0]; i1 = i1 + patch_group0->patch[r]->size[0])
          {
            ...
            for (v = wavelet_rst_id->fiwavelet_rst_index; v <= rst_id->last_index; ++v)
            {
              PIDX_variable var = var_grp->variable[v];
              Ndim_patch out_patch = var->rst_patch_group[g]->reg_patch;
