...
static PIDX_return_code print_global_data (PIDX_wavelet_id file, int gi, int v);
static PIDX_return_code create_debug_comm (PIDX_wavelet_id file, int gi, int v);
//static PIDX_return_code create_wavelet_buffers (PIDX_wavelet_id file, int gi, int v);

static PIDX_return_code calculate_neighbor_ranks (PIDX_wavelet_id file);
static PIDX_return_code calculate_neighbor_ranks_x (PIDX_wavelet_id file);
static PIDX_return_code calculate_neighbor_ranks_y (PIDX_wavelet_id file);
static PIDX_return_code calculate_neighbor_ranks_z (PIDX_wavelet_id file);

static PIDX_return_code create_stencil_buffers (PIDX_wavelet_id file, int gi, int v, int level);
static PIDX_return_code destroy_stencil_buffers();

static PIDX_return_code wavelet_odd_x (PIDX_wavelet_id file, int gi, int v, int level, int x_stride_x, int x_stride_y, int x_stride_z);
static PIDX_return_code wavelet_comm_p2p_odd_x (PIDX_wavelet_id file, int gi, int v, int x_stride_y, int x_stride_z);
static PIDX_return_code wavelet_comp_odd_x (PIDX_wavelet_id file, int gi, int v, int l, int x_stride_x, int x_stride_y, int x_stride_z);

static PIDX_return_code wavelet_even_x (PIDX_wavelet_id file, int gi, int v, int level, int x_stride_x, int x_stride_y, int x_stride_z);
static PIDX_return_code wavelet_comm_p2p_even_x (PIDX_wavelet_id file, int gi, int v, int l, int x_stride_y, int x_stride_z);
static PIDX_return_code wavelet_comp_even_x (PIDX_wavelet_id file, int gi, int v, int l, int x_stride_x, int x_stride_y, int x_stride_z);

static PIDX_return_code wavelet_odd_y (PIDX_wavelet_id file, int gi, int v, int level, int y_stride_x, int y_stride_y, int y_stride_z);
static PIDX_return_code wavelet_comm_p2p_odd_y (PIDX_wavelet_id file, int gi, int v, int y_stride_y, int y_stride_z);
static PIDX_return_code wavelet_comp_odd_y (PIDX_wavelet_id file, int gi, int v, int l, int y_stride_x, int y_stride_y, int y_stride_z);

static PIDX_return_code wavelet_even_y (PIDX_wavelet_id file, int gi, int v, int level, int y_stride_x, int y_stride_y, int y_stride_z);
static PIDX_return_code wavelet_comm_p2p_even_y (PIDX_wavelet_id file, int gi, int v, int level, int y_stride_y, int y_stride_z);
static PIDX_return_code wavelet_comp_even_y (PIDX_wavelet_id file, int gi, int v, int l, int y_stride_x, int y_stride_y, int y_stride_z);

static PIDX_return_code wavelet_odd_z (PIDX_wavelet_id file, int gi, int v, int level, int stride_x, int stride_y, int stride_z);
static PIDX_return_code wavelet_comm_p2p_odd_z (PIDX_wavelet_id file, int gi, int v, int stride_y, int stride_z);
static PIDX_return_code wavelet_comp_odd_z (PIDX_wavelet_id file, int gi, int v, int l, int stride_x, int stride_y, int stride_z);

static PIDX_return_code wavelet_even_z (PIDX_wavelet_id file, int gi, int v, int level, int stride_x, int stride_y, int stride_z);
static PIDX_return_code wavelet_comm_p2p_even_z (PIDX_wavelet_id file, int gi, int v, int level, int stride_y, int stride_z);
...
PIDX_return_code idx_stencil_wavelet(PIDX_wavelet_id file, int gi, int svi, int evi, int mode)
{
  ...
  for (v = svi; v <= evi; v++)
  {
    ...
    assert(var->restructured_super_patch_count <= 1);
    ...
    if (var->restructured_super_patch_count == 0)
      goto comm_cleanup;
...
#endif
#if 1
    for (l = 0; l < file->idx_derived->wavelet_levels; l++)
    {
      ...
      if ((int) var->restructured_super_patch->restructured_patch->size[0] >= (int)pow(2, l + 1))
      {
      ...
      if ((int) var->restructured_super_patch->restructured_patch->size[1] >= (int)pow(2, l + 1))
      {
      ...
      if ((int) var->restructured_super_patch->restructured_patch->size[2] >= (int)pow(2, l + 1))
      {
...
static PIDX_return_code create_wavelet_buffers (PIDX_wavelet_id file, int gi, int v)
{
  ...
  var->restructured_super_patch->wavelet_reg_patch = malloc(sizeof(*(var->restructured_super_patch->wavelet_reg_patch)));

  memcpy(var->restructured_super_patch->wavelet_reg_patch->offset, var->restructured_super_patch->restructured_patch->offset, sizeof(unsigned long long) * PIDX_MAX_DIMENSIONS);
  memcpy(var->restructured_super_patch->wavelet_reg_patch->size, var->restructured_super_patch->restructured_patch->size, sizeof(unsigned long long) * PIDX_MAX_DIMENSIONS);

  int s_x = (int) var->restructured_super_patch->wavelet_reg_patch->size[0];
  int s_y = (int) var->restructured_super_patch->wavelet_reg_patch->size[1];
  int s_z = (int) var->restructured_super_patch->wavelet_reg_patch->size[2];

  var->restructured_super_patch->wavelet_reg_patch->buffer = malloc(s_x * s_y * s_z * (var->bpv/8) * var->vps);
  if (var->restructured_super_patch->wavelet_reg_patch->buffer == NULL)
  {
  ...
  memcpy(var->restructured_super_patch->wavelet_reg_patch->buffer, var->restructured_super_patch->restructured_patch->buffer, s_x * s_y * s_z * (var->bpv/8) * var->vps);
...
static PIDX_return_code calculate_neighbor_ranks_x (PIDX_wavelet_id file)
{
  ...
  p_rank = file->idx_c->grank_x + file->idx_c->gnproc_x / file->idx_derived->restructured_grid->total_patch_count[0];
  if (p_rank >= file->idx_c->gnproc_x)
    p_rank = -1;

  n_rank = file->idx_c->grank_x - file->idx_c->gnproc_x / file->idx_derived->restructured_grid->total_patch_count[0];
  if (n_rank < 0)
    n_rank = -1;
...
static PIDX_return_code calculate_neighbor_ranks_y (PIDX_wavelet_id file)
{
  ...
  p_rank = file->idx_c->grank_y + file->idx_c->gnproc_y / file->idx_derived->restructured_grid->total_patch_count[1];
  if (p_rank >= file->idx_c->gnproc_y)
    p_rank = -1;

  n_rank = file->idx_c->grank_y - file->idx_c->gnproc_y / file->idx_derived->restructured_grid->total_patch_count[1];
  if (n_rank < 0)
    n_rank = -1;
...
static PIDX_return_code calculate_neighbor_ranks_z (PIDX_wavelet_id file)
{
  ...
  p_rank = file->idx_c->grank_z + file->idx_c->gnproc_z / file->idx_derived->restructured_grid->total_patch_count[2];
  if (p_rank >= file->idx_c->gnproc_z)
    p_rank = -1;

  n_rank = file->idx_c->grank_z - file->idx_c->gnproc_z / file->idx_derived->restructured_grid->total_patch_count[2];
  if (n_rank < 0)
    n_rank = -1;
...
static PIDX_return_code create_stencil_buffers (PIDX_wavelet_id file, int gi, int v, int l)
{
  ...
  int s_x = (int) var->restructured_super_patch->restructured_patch->size[0];
  int s_y = (int) var->restructured_super_patch->restructured_patch->size[1];
  int s_z = (int) var->restructured_super_patch->restructured_patch->size[2];
...
static PIDX_return_code wavelet_odd_x (PIDX_wavelet_id file, int gi, int v, int l, int x_stride_x, int x_stride_y, int x_stride_z)
{
  PIDX_time time = file->idx_derived->time;
...
static PIDX_return_code wavelet_even_x (PIDX_wavelet_id file, int gi, int v, int l, int x_stride_x, int x_stride_y, int x_stride_z)
{
  PIDX_time time = file->idx_derived->time;
...
static PIDX_return_code wavelet_odd_y (PIDX_wavelet_id file, int gi, int v, int l, int y_stride_x, int y_stride_y, int y_stride_z)
{
  PIDX_time time = file->idx_derived->time;
...
static PIDX_return_code wavelet_even_y (PIDX_wavelet_id file, int gi, int v, int l, int stride_x, int stride_y, int stride_z)
{
  PIDX_time time = file->idx_derived->time;
...
static PIDX_return_code wavelet_odd_z (PIDX_wavelet_id file, int gi, int v, int l, int stride_x, int stride_y, int stride_z)
{
  PIDX_time time = file->idx_derived->time;
...
static PIDX_return_code wavelet_even_z (PIDX_wavelet_id file, int gi, int v, int l, int stride_x, int stride_y, int stride_z)
{
  PIDX_time time = file->idx_derived->time;
...
static PIDX_return_code wavelet_comm_p2p_odd_x (PIDX_wavelet_id file, int gi, int v, int x_stride_y, int x_stride_z)
{
  ...
  int s_x = (int) var->restructured_super_patch->restructured_patch->size[0] / file->idx->chunk_size[0];
  int s_y = (int) var->restructured_super_patch->restructured_patch->size[1] / file->idx->chunk_size[1];
  int s_z = (int) var->restructured_super_patch->restructured_patch->size[2] / file->idx->chunk_size[2];
  unsigned char* wb = var->restructured_super_patch->restructured_patch->buffer;

  //fprintf(stderr, "My rank %d [%d + (%d/%d)] Rank I get data from %d Rank I send data to %d\n", file->idx_c->grank, file->idx_c->grank_x, file->idx_c->gnproc_x, file->idx_derived->restructured_grid->total_patch_count[0], positive_rank_x, negative_rank_x);
...
static PIDX_return_code wavelet_comm_p2p_odd_y (PIDX_wavelet_id file, int gi, int v, int y_stride_x, int y_stride_z)
{
  MPI_Request request[2];
  int i = 0, j = 0, k = 0;
  PIDX_variable_group var_grp = file->idx->variable_grp[gi];
...
static PIDX_return_code wavelet_comp_odd_y (PIDX_wavelet_id file, int gi, int v, int l, int stride_x, int stride_y, int stride_z)
{
  PIDX_variable_group var_grp = file->idx->variable_grp[gi];
  PIDX_variable var = var_grp->variable[v];
  int chunk_size = file->idx->chunk_size[0] * file->idx->chunk_size[1] * file->idx->chunk_size[2];
...
static PIDX_return_code wavelet_comm_p2p_even_y (PIDX_wavelet_id file, int gi, int v, int l, int stride_x, int stride_z)
{
  int ret = 0;
  int count = 0;
  int lindex = 0;
...
static PIDX_return_code wavelet_comp_even_y (PIDX_wavelet_id file, int gi, int v, int l, int stride_x, int stride_y, int stride_z)
{
  int sample_count = 0;
  int interval = (int)pow(2,l);
  int up_index = 0;
...
static PIDX_return_code wavelet_comm_p2p_odd_z (PIDX_wavelet_id file, int gi, int v, int stride_x, int stride_y)
{
  MPI_Request request[2];
  int i = 0, j = 0, k = 0;
  PIDX_variable_group var_grp = file->idx->variable_grp[gi];
...
static PIDX_return_code wavelet_comm_p2p_even_z (PIDX_wavelet_id file, int gi, int v, int l, int stride_x, int stride_y)
{
  int ret = 0;
  int count = 0;
  int lindex = 0;
...
static PIDX_return_code wavelet_comp_even_z (PIDX_wavelet_id file, int gi, int v, int l, int stride_x, int stride_y, int stride_z)
{
  ...
  int s_x = (int) var->restructured_super_patch->restructured_patch->size[0] / file->idx->chunk_size[0];
  int s_y = (int) var->restructured_super_patch->restructured_patch->size[1] / file->idx->chunk_size[0];
  int s_z = (int) var->restructured_super_patch->restructured_patch->size[2] / file->idx->chunk_size[0];
  unsigned char* wb = var->restructured_super_patch->restructured_patch->buffer;
...
static PIDX_return_code print_global_data (PIDX_wavelet_id file, int gi, int v)
{
  ...
  int o_x = (int) var->restructured_super_patch->restructured_patch->offset[0] / file->idx->chunk_size[0];
  int o_y = (int) var->restructured_super_patch->restructured_patch->offset[1] / file->idx->chunk_size[1];
  int o_z = (int) var->restructured_super_patch->restructured_patch->offset[2] / file->idx->chunk_size[2];
