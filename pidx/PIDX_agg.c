...
int aggregate_write_read(PIDX_agg_id agg_id, Agg_buffer agg_buffer, int variable_index, uint64 hz_start_index, uint64 hz_count, unsigned char* hz_buffer, int buffer_offset, int MODE)
{
  int ret;
  int rank = 0, itr;
  ...
  int target_rank = 0;
  int64 start_agg_index = 0, end_agg_index = 0, target_disp = 0, target_count = 0, hz_start = 0, samples_in_file = 0;
  int64 samples_per_file = (int64) agg_id->idx_derived_ptr->samples_per_block * agg_id->idx_ptr->blocks_per_file;
  ...
  start_agg_index = target_disp / (int64) (samples_in_file / agg_id->idx_derived_ptr->aggregation_factor);
  end_agg_index = ((target_disp + target_count - 1) / (int64) (samples_in_file / agg_id->idx_derived_ptr->aggregation_factor));
...
int PIDX_agg_aggregate(PIDX_agg_id agg_id, Agg_buffer agg_buffer) 
{
  ...
  {
    agg_buffer->rank_holder[i - agg_id->start_var_index] = malloc( agg_id->idx_ptr->variable[i]->values_per_sample  * sizeof (int*) * agg_id->idx_derived_ptr->aggregation_factor);
    ...
    for (j = 0; j < agg_id->idx_ptr->variable[i]->values_per_sample * agg_id->idx_derived_ptr->aggregation_factor; j++)
    {
      for (k = 0; k < agg_id->idx_ptr->variable[i]->VAR_existing_file_count; k++)
      {
        ...
        if(rank == agg_buffer->rank_holder[i - agg_id->start_var_index][j][agg_id->idx_ptr->variable[i]->VAR_existing_file_index[k]])
        {
          ...
          agg_buffer->buffer_size = agg_id->idx_ptr->variable[agg_buffer->var_number]->VAR_blocks_per_file[agg_buffer->file_number] * (agg_id->idx_derived_ptr->samples_per_block / agg_id->idx_derived_ptr->aggregation_factor) * (agg_id->idx_ptr->variable[agg_buffer->var_number]->bits_per_value/8);
...
int PIDX_agg_aggregate_write_read(PIDX_agg_id agg_id, Agg_buffer agg_buffer, int MODE)
{
  ...
  int send_index = 0;
  int64 index = 0, count = 0, hz_index = 0;
