...
static PIDX_return_code local_aggregate_write_read(PIDX_agg_id agg_id, int variable_index, uint64_t hz_start_index, uint64_t hz_count, unsigned char* hz_buffer, int buffer_offset, Agg_buffer agg_buffer, PIDX_block_layout block_layout, int MODE);
static PIDX_return_code local_aggregate_write_read_layout_zero(PIDX_agg_id agg_id, int variable_index, uint64_t hz_start_index, uint64_t hz_count, unsigned char* hz_buffer, int buffer_offset, Agg_buffer agg_buffer, PIDX_block_layout block_layout, MPI_Datatype, int MODE);
...
static PIDX_return_code layout_zero(PIDX_agg_id agg_id, Agg_buffer agg_buffer, PIDX_block_layout block_layout, int mode)
{
  ...
  for(v = agg_id->first_index; v <= agg_id->last_index; v++)
  {
    ...
    for (p = 0; p < var->patch_group_count; p++)
    {
      ...
      MPI_Type_indexed( (block_layout->resolution_to - block_layout->resolution_from), hz_buf->lower_hz_count, hz_buf->lower_hz_disp, MPI_DOUBLE, &chunk_data_type);
...
#if !SIMULATE_IO
      ret = local_aggregate_write_read_layout_zero(agg_id, v, /*hz_buf->start_hz_index[i]*/0, total_count, agg_id->idx->variable[v]->hz_buffer[p]->lower_hz_buffer, 0, agg_buffer, block_layout, chunk_data_type, mode);
#else
          ...
          if (hz_buf->samples_per_level[i] != 0)
          {
            for(e1 = 0; e1 < hz_buf->samples_per_level[i] ; e1++)
            {
              if(e1 == 0)
              {
                ...
                if(hz_buf->samples_per_level[i] == 1)
                {
                  ret = local_aggregate_write_read(agg_id, v, index, count, agg_id->idx->variable[v]->hz_buffer[p]->buffer[i], send_index, agg_buffer, block_layout, PIDX_WRITE);
                  if (ret != PIDX_success)
                  {
                    fprintf(stderr, " Error in local_aggregate_write_read Line %d File %s\n", __LINE__, __FILE__);
                    return PIDX_err_agg;
              ...
              else
              {
                if(hz_buf->buffer_index[hz_index] - hz_buf->buffer_index[hz_index - 1] == 1)
                {
                  ...
                  if(e1 == hz_buf->samples_per_level[i] - 1)
                  {
                    local_aggregate_write_read(agg_id, v, index, count, agg_id->idx->variable[v]->hz_buffer[p]->buffer[i], send_index, agg_buffer, block_layout, PIDX_WRITE);
                    if (ret != PIDX_success)
                    {
                      fprintf(stderr, " Error in local_aggregate_write_read Line %d File %s\n", __LINE__, __FILE__);
                      return PIDX_err_agg;
                ...
                else
                {
                  ...
                  if(e1 == hz_buf->samples_per_level[i] - 1)
                  {
                    local_aggregate_write_read(agg_id, v, hz_buf->buffer_index[hz_index], 1, agg_id->idx->variable[v]->hz_buffer[p]->buffer[i], e1, agg_buffer, block_layout, PIDX_WRITE);
                    if (ret != PIDX_success)
                    {
                      fprintf(stderr, " Error in local_aggregate_write_read Line %d File %s\n", __LINE__, __FILE__);
                      return PIDX_err_agg;
      ...
      else if (hz_buf->type == 1)
      {
        ...
        for (i = block_layout->resolution_from; i < block_layout->resolution_to; i++)
        {
          if (hz_buf->nsamples_per_level[i][0] * hz_buf->nsamples_per_level[i][1] * hz_buf->nsamples_per_level[i][2] != 0)
          {
            ...
            //memcpy(&x, agg_id->idx->variable[v]->hz_buffer[p]->buffer[i], sizeof(double));
            //printf("offset %d count %d val %f\n", hz_buf->start_hz_index[i], count, x);
            ret = local_aggregate_write_read(agg_id, v, hz_buf->start_hz_index[i], count, agg_id->idx->variable[v]->hz_buffer[p]->buffer[i], 0, agg_buffer, block_layout, mode);
#else
            ret = local_aggregate_write_read(agg_id, v, hz_buf->start_hz_index[i], count, NULL, 0, agg_buffer, block_layout, mode);
#endif
            if (ret != PIDX_success)
...
#if !SIMULATE_IO
              ret = local_aggregate_write_read(agg_id, v, var0->hz_buffer[p]->start_hz_index[i], count, hz_buf->buffer[i], 0, agg_buffer, block_layout, mode);
              if (ret != PIDX_success)
              {
                fprintf(stderr, " Error in local_aggregate_write_read Line %d File %s\n", __LINE__, __FILE__);
                return PIDX_err_agg;
...
#else
              ret = local_aggregate_write_read(agg_id, v, var0->hz_buffer[p]->start_hz_index[i], count, NULL, 0, agg_buffer, block_layout, mode);
              if (ret != PIDX_success)
              {
                fprintf(stderr, " Error in local_aggregate_write_read Line %d File %s\n", __LINE__, __FILE__);
                return PIDX_err_agg;
...
#if !SIMULATE_IO
                  ret = local_aggregate_write_read(agg_id, v, index + agg_id->idx->variable[v]->hz_buffer[p]->start_hz_index[i], count, agg_id->idx->variable[v]->hz_buffer[p]->buffer[i], send_index, agg_buffer, block_layout, mode);
                  if (ret != PIDX_success)
                  {
                    fprintf(stderr, "[%s] [%d] write_read_samples() failed.\n", __FILE__, __LINE__);
                    return PIDX_err_agg;
...
#else
                  ret = local_aggregate_write_read(agg_id, v, index + agg_id->idx->variable[v]->hz_buffer[p]->start_hz_index[i], count, NULL, send_index, agg_buffer, block_layout, mode);
                  if (ret != PIDX_success)
                  {
                    fprintf(stderr, "[%s] [%d] write_read_samples() failed.\n", __FILE__, __LINE__);
                    return PIDX_err_agg;
...
static PIDX_return_code local_aggregate_write_read_layout_zero(PIDX_agg_id agg_id, int variable_index, uint64_t hz_start_index, uint64_t hz_count, unsigned char* hz_buffer, int buffer_offset, Agg_buffer agg_buffer, PIDX_block_layout block_layout, MPI_Datatype chunk_type, int MODE)
{
  int rank = 0, itr;// nrank = 0;
  int bytes_per_datatype;
  int file_no = 0, block_no = 0, negative_block_offset = 0, sample_index = 0, values_per_sample;
...
{
  int ret;
  ret = create_window(agg_id, agg_buffer);
  if (ret != PIDX_success)
  {
...
PIDX_return_code PIDX_local_agg_write(PIDX_agg_id agg_id, Agg_buffer agg_buffer, int layout_id, PIDX_block_layout block_layout)
{
  ...
  if (layout_id == 0)
  {
    ret = layout_zero(agg_id, agg_buffer, block_layout, PIDX_WRITE);
    if (ret != PIDX_success)
    {
      fprintf(stderr, " [%s] [%d] Fence error.\n", __FILE__, __LINE__);
      return PIDX_err_agg;
