...
static int populate_meta_data(PIDX_header_io_id header_io_id, int file_number, char* bin_file)
{
  ...
  for (n = 0; n < header_io_id->end_var_index; n++)
  {
	...
	printf("[%d] [%d] [%d] (%ld) :: [%d %d %d] Offste and Count %d %d\n", 
	       ...
	       n,
	i,
	header_io_id->idx_derived_ptr->start_fs_block,
	header_io_id->idx_derived_ptr->samples_per_block,
	bytes_per_sample,
	header_io_id->idx_ptr->variable[n]->values_per_sample,
	little_data_offset,
	header_io_id->idx_derived_ptr->samples_per_block * bytes_per_sample * header_io_id->idx_ptr->variable[n]->values_per_sample);
