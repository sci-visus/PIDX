...
static PIDX_return_code wavelet_rst_x (PIDX_io file, int gi, int v, int l, int stride_x, int stride_y, int stride_z);
static PIDX_return_code wavelet_rst_y (PIDX_io file, int gi, int v, int l, int stride_x, int stride_y, int stride_z);
static PIDX_return_code wavelet_rst_z (PIDX_io file, int gi, int v, int l, int stride_x, int stride_y, int stride_z);


PIDX_return_code idx_rst_wavelet(PIDX_io file, int gi, int svi, int evi, int mode)
{
  int v = 0;
  int l = 0;
  int i1 = 0, j1 = 0, k1 = 0;
  ...
  for (v = svi; v <= evi; v++)
  {
    ...
    for (l = 0; l < file->idx_d->wavelet_levels; l++)
    {
      ...
      if ((int) patch_group->reg_patch->size[0] >= (int)pow(2, l + 1))
      {
        ...
        if (wavelet_rst_x (file, gi, v, l, x_stride_x, x_stride_y, x_stride_z) != PIDX_success)
        {
          fprintf(stderr, "Error: File [%s] Line [%d]\n", __FILE__, __LINE__);
          return PIDX_err_wavelet;
        }
      ...
      if ((int) patch_group->reg_patch->size[1] >= (int)pow(2, l + 1))
      {
        ...
        if (wavelet_rst_y (file, gi, v, l, y_stride_x, y_stride_y, y_stride_z) != PIDX_success)
        {
          fprintf(stderr, "Error: File [%s] Line [%d]\n", __FILE__, __LINE__);
          return PIDX_err_wavelet;
        }
      ...
      if ((int) patch_group->reg_patch->size[2] >= (int)pow(2, l + 1))
      {
        ...
        if (wavelet_rst_z (file, gi, v, l, z_stride_x, z_stride_y, z_stride_z) != PIDX_success)
        {
          fprintf(stderr, "Error: File [%s] Line [%d]\n", __FILE__, __LINE__);
          return PIDX_err_wavelet;
        }
...
static PIDX_return_code wavelet_rst_x (PIDX_io file, int gi, int v, int l, int stride_x, int stride_y, int stride_z)
{
  ...
  for (i = 0; i < (int) var->rst_patch_group[0]->reg_patch->offset[1] + (int) var->rst_patch_group[0]->reg_patch->size[1] - 1; i = i + stride_y)
  {
  ...
  for (i = 0; i < (int) var->rst_patch_group[0]->reg_patch->offset[2] + (int) var->rst_patch_group[0]->reg_patch->size[2] - 1; i = i + stride_z)
  {
  ...
  for (k = z_start_offset; k < s_z - 0; k = k + stride_z)
  {
    for (j = y_start_offset; j < s_y - 0; j =  j + stride_y)
    {
...
static PIDX_return_code wavelet_rst_y (PIDX_io file, int gi, int v, int l, int stride_x, int stride_y, int stride_z)
{
  ...
  for (i = 0; i < (int) var->rst_patch_group[0]->reg_patch->offset[0] + (int) var->rst_patch_group[0]->reg_patch->size[0] - 1; i = i + stride_x)
  {
    if (i >= (int) var->rst_patch_group[0]->reg_patch->offset[0])
    {
      //if (file->idx_c->grank == 1)
  ...
  for (k = z_start_offset; k < s_z - 0; k = k + stride_z)
  {
    for (i = x_start_offset ; i < s_x - 0; i = i + stride_x)
    {
      // every sample but the last
