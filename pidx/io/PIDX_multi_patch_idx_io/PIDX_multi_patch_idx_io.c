...
static PIDX_return_code populate_idx_dataset(PIDX_multi_patch_idx_io file, int start_var_index, int end_var_index);
...
struct PIDX_multi_patch_idx_io_descriptor
{
  ...
  if (file->idx_d->parallel_mode == 1)
  {
    ...
    if (file->idx->enable_rst == 1)
    {
      ...
      if (total_patch_count != nprocs){
        ...
        printf("Disabling restructuring because patch count != total number of processes\n");
      }
#endif
...
static void set_default_patch_size(PIDX_multi_patch_idx_io file, int64_t* process_bounds, int nprocs)
{
  int i = 0, j = 0;
...
static PIDX_return_code populate_idx_file_structure(PIDX_multi_patch_idx_io file)
{
...
static PIDX_return_code delete_idx_dataset(PIDX_multi_patch_idx_io file, int start_index, int end_index)
{
...
PIDX_return_code PIDX_file_initialize_time_step(PIDX_multi_patch_idx_io file, char* filename, int current_time_step)
{
  int N;
...
static PIDX_return_code populate_idx_dataset(PIDX_multi_patch_idx_io file, int start_index, int end_index)
{
  int i = 0, j = 0, ctr;
  int file_number = 0;
...
PIDX_multi_patch_idx_io PIDX_multi_patch_idx_io_init( idx_dataset idx_meta_data, idx_dataset_derived_metadata idx_derived_ptr, idx_debug idx_dbg)
{
  //Creating the restructuring ID
  PIDX_multi_patch_idx_io multi_patch_idx_io_id;
  multi_patch_idx_io_id = malloc(sizeof (*multi_patch_idx_io_id));
  memset(multi_patch_idx_io_id, 0, sizeof (*multi_patch_idx_io_id));

  multi_patch_idx_io_id->idx = idx_meta_data;
  multi_patch_idx_io_id->idx_d = idx_derived_ptr;
  multi_patch_idx_io_id->idx_dbg = idx_dbg;

  return (multi_patch_idx_io_id);
}


#if PIDX_HAVE_MPI
PIDX_return_code PIDX_multi_patch_idx_io_set_communicator(PIDX_multi_patch_idx_io id, MPI_Comm comm)
{
  if (id == NULL)
    return PIDX_err_id;
...
PIDX_return_code PIDX_multi_patch_idx_write(PIDX_multi_patch_idx_io file, int start_var_index, int end_var_index)
{
...
//    memset(max_union_patch_size, 0, (sizeof (int64_t) * PIDX_MAX_DIMENSIONS));
//    
//    MPI_Allreduce(union_patch_size, max_union_patch_size, PIDX_MAX_DIMENSIONS, MPI_INT64_T, MPI_MAX, file->comm);
//    
//     printf("max union size %lld %lld %lld %lld %lld\n", max_union_patch_size[0],max_union_patch_size[1],max_union_patch_size[2],max_union_patch_size[3],max_union_patch_size[4] );
  ...
  for (start_index = start_var_index; start_index < end_var_index; start_index = start_index + (file->idx_d->var_pipe_length + 1))
  {
    ...
    if (file->idx_dbg->debug_do_rst == 1)
    {
      ret = PIDX_multi_patch_rst_write(file->multi_patch_rst_id);
      if (ret != PIDX_success)
        return PIDX_err_rst;
    }
#endif
    ...
    if (file->idx_dbg->debug_do_agg == 1)
    {
      ...
      for(i = start_index ; i < (end_index + 1) ; i = i + (agg_var_pipe + 1))
      {
        ...
        //for(j = agg_io_level - 1 ; j < agg_io_level; j++)
        {
           ...
           // TODO commented temporary, the signature has been changed
...
PIDX_return_code PIDX_multi_patch_idx_read(PIDX_multi_patch_idx_io file, int start_var_index, int end_var_index)
{
  file->idx_d->var_pipe_length = file->idx->variable_count - 1;
  if (file->idx_d->var_pipe_length == 0)
    file->idx_d->var_pipe_length = 1;
...
PIDX_return_code PIDX_multi_patch_idx_io_finalize(PIDX_multi_patch_idx_io file)
{
#if PIDX_HAVE_MPI
