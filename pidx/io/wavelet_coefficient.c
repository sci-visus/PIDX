...
static PIDX_return_code p2p_positive_x (PIDX_io file, int gi, int v, int level);
static PIDX_return_code wavelet_odd_x (PIDX_io file, int gi, int v, int l);
static PIDX_return_code p2p_negative_x (PIDX_io file, int gi, int v, int level);
static PIDX_return_code wavelet_even_x (PIDX_io file, int gi, int v, int l);

static PIDX_return_code p2p_positive_y (PIDX_io file, int gi, int v, int level);
static PIDX_return_code wavelet_odd_y (PIDX_io file, int gi, int v, int l);
static PIDX_return_code p2p_negative_y (PIDX_io file, int gi, int v, int level);
static PIDX_return_code wavelet_even_y (PIDX_io file, int gi, int v, int l);

static PIDX_return_code p2p_positive_z (PIDX_io file, int gi, int v, int level);
static PIDX_return_code wavelet_odd_z (PIDX_io file, int gi, int v, int l);
static PIDX_return_code p2p_negative_z (PIDX_io file, int gi, int v, int level);
...
PIDX_return_code wavelet(PIDX_io file, int gi, int svi, int evi, int mode)
{
  ...
  for (v = svi; v <= evi; v++)
  {
    ...
    for (l = 0; l < l_x; l++)
    {
      ...
      ret = p2p_positive_x (file, gi, v, l);
      if (ret != PIDX_success)
      {
        fprintf(stderr, "Error: File [%s] Line [%d]\n", __FILE__, __LINE__);
        return PIDX_err_wavelet;
...
#endif
      ret = p2p_negative_x (file, gi, v, l);
      if (ret != PIDX_success)
      {
        fprintf(stderr, "Error: File [%s] Line [%d]\n", __FILE__, __LINE__);
        return PIDX_err_wavelet;
...
#if 1
      ret = p2p_positive_y (file, gi, v, l);
      if (ret != PIDX_success)
      {
        fprintf(stderr, "Error: File [%s] Line [%d]\n", __FILE__, __LINE__);
        return PIDX_err_wavelet;
      ...
      ret = p2p_negative_y (file, gi, v, l);
      if (ret != PIDX_success)
      {
        fprintf(stderr, "Error: File [%s] Line [%d]\n", __FILE__, __LINE__);
        return PIDX_err_wavelet;
...
#if 1
      ret = p2p_positive_z (file, gi, v, l);
      if (ret != PIDX_success)
      {
        fprintf(stderr, "Error: File [%s] Line [%d]\n", __FILE__, __LINE__);
        return PIDX_err_wavelet;
...
#endif
      ret = p2p_negative_z (file, gi, v, l);
      if (ret != PIDX_success)
      {
        fprintf(stderr, "Error: File [%s] Line [%d]\n", __FILE__, __LINE__);
        return PIDX_err_wavelet;
...
static PIDX_return_code calculate_neighbor_ranks_y (PIDX_io file)
{
  ...
  if (p_rank != -1)
    positive_rank_y = (file->idx_c->grank_z * file->idx_c->gnproc_y * file->idx_c->gnproc_y) +
                      (p_rank * file->idx_c->gnproc_y) +
                       file->idx_c->grank_x;
  else
    positive_rank_y = -1;

  if (n_rank != -1)
    negative_rank_y = (file->idx_c->grank_z * file->idx_c->gnproc_y * file->idx_c->gnproc_y) +
                      (n_rank * file->idx_c->gnproc_y) +
                       file->idx_c->grank_x;
  else
    negative_rank_y = -1;
...
static PIDX_return_code calculate_neighbor_ranks_z (PIDX_io file)
{
  ...
  if (p_rank != -1)
    positive_rank_z = (p_rank * file->idx_c->gnproc_z * file->idx_c->gnproc_z) +
                      (file->idx_c->grank_z * file->idx_c->gnproc_z) +
                       file->idx_c->grank_x;
  else
    positive_rank_z = -1;

  if (n_rank != -1)
    negative_rank_z = (n_rank * file->idx_c->gnproc_z * file->idx_c->gnproc_z) +
                      (file->idx_c->grank_z * file->idx_c->gnproc_z) +
                       file->idx_c->grank_x;
  else
    negative_rank_z = -1;
...
static PIDX_return_code p2p_positive_x (PIDX_io file, int gi, int v, int l)
{
...
static PIDX_return_code p2p_negative_x (PIDX_io file, int gi, int v, int l)
{
  int ret = 0;
  int count = 0;
  int lindex = 0;
...
static PIDX_return_code p2p_positive_y (PIDX_io file, int gi, int v, int l)
{
  MPI_Request request[2];
  int i = 0, j = 0, k = 0;
  int stride = (int)pow(2, l);
...
static PIDX_return_code p2p_negative_y (PIDX_io file, int gi, int v, int l)
{
  int ret = 0;
  int count = 0;
  int lindex = 0;
...
static PIDX_return_code p2p_positive_z (PIDX_io file, int gi, int v, int l)
{
  MPI_Request request[2];
  int i = 0, j = 0, k = 0;
  int stride = (int)pow(2, l);
...
static PIDX_return_code wavelet_odd_z (PIDX_io file, int gi, int v, int l)
{
  ...
  for (j = 0; j < s_y; j = j + stride)
  {
    ...
    {
      // every sample but the last
      for (k = odd_start_offset ; k < s_z - stride_z; j =  j + stride_z)
      {
         index = (s_x * s_y * k) + (s_x * j) + i;
...
static PIDX_return_code p2p_negative_z (PIDX_io file, int gi, int v, int l)
{
  int ret = 0;
  int count = 0;
  int lindex = 0;
...
static PIDX_return_code wavelet_even_z (PIDX_io file, int gi, int v, int l)
{
  ...
  for (j = 0; j < s_y; j = j + stride)
  {
    for (i = 0; i < s_x; i = i + stride)
    {
      for (k = stride_z; j < s_z; j = j + stride_z)
      {
         index = (s_x * s_y * k) + (s_x * j) + i;
         up_index = (s_x * s_y * (k + interval)) + (s_x * j) + i;
         down_index = (s_x * s_y * (k - interval)) + (s_x * j) + i;
