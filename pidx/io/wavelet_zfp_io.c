...
PIDX_return_code PIDX_wavelet_zfp_write(PIDX_io file, int gi, int svi, int evi)
{
  int li = 0;
  int si = 0, ei = 0;
  ...
  file->idx->variable_count = file->idx->variable_count + 1;
  file->idx->variable_grp[0]->variable_count = file->idx->variable_count;
  file->idx_d->var_pipe_length = file->idx->variable_count - 1 > 1 ? file->idx->variable_count - 1 : 1;

  PIDX_variable variable = malloc(sizeof (*variable));
  memset(variable, 0, sizeof (*variable));

  variable->vps = 1;
  variable->bpv = (sizeof(float) * 8);
  variable->sim_patch_count = 0;
  strcpy(variable->type_name, "1*float32");
  strcpy(variable->var_name, "Average_field");
  ...
  variable->sim_patch[variable->sim_patch_count]->buffer = file->idx->variable_grp[0]->variable[0]->sim_patch[0]->buffer;

  memcpy(variable->sim_patch[variable->sim_patch_count]->offset, file->idx->variable_grp[0]->variable[0]->sim_patch[0]->offset, PIDX_MAX_DIMENSIONS * sizeof(unsigned long long));
  memcpy(variable->sim_patch[variable->sim_patch_count]->size, file->idx->variable_grp[0]->variable[0]->sim_patch[0]->size, PIDX_MAX_DIMENSIONS * sizeof(unsigned long long));
