...
PIDX_return_code PIDX_idx_write(PIDX_hybrid_idx_io file, int gi, int svi, int evi)
{
  ...
  ret = one_time_initialize(file, PIDX_WRITE, PIDX_IDX_IO);
  if (ret != PIDX_success) {fprintf(stdout,"File %s Line %d\n", __FILE__, __LINE__); return PIDX_err_file;}
...
PIDX_return_code PIDX_idx_read(PIDX_hybrid_idx_io file, int gi, int svi, int evi)
{
  PIDX_return_code ret;
  int start_index = 0;
  ...
  // calculates maxh and bitstring
  time->idx_bit_string_start = PIDX_get_time();
  ret = populate_bit_string(file, PIDX_READ, PIDX_IDX_IO);
  if (ret != PIDX_success) {fprintf(stdout,"File %s Line %d\n", __FILE__, __LINE__); return PIDX_err_file;}
  ...
  // computes the filename temlates
  ret = one_time_initialize(file, PIDX_READ, PIDX_IDX_IO);
  if (ret != PIDX_success) {fprintf(stdout,"File %s Line %d\n", __FILE__, __LINE__); return PIDX_err_file;}
  time->idx_bit_string_end = PIDX_get_time();
