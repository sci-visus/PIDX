...
PIDX_return_code PIDX_file_create(const char* filename, PIDX_flags flags, PIDX_access access_type, PIDX_file* file)
{
  ...
  *file = malloc(sizeof (*(*file)) );
  ...
  (*file)->idx_ptr->global_bounds = malloc(sizeof(int64) * PIDX_MAX_DIMENSIONS);
  ...
  if (access_type->parallel)
  {
    ...
    MPI_Comm_size(access_type->comm, &nprocs);
  }
#endif
...
PIDX_return_code PIDX_set_dims(PIDX_file file, PIDX_point dims)
{
  ...
  memcpy(file->idx_ptr->global_bounds, dims, PIDX_MAX_DIMENSIONS * sizeof(int64));
...
PIDX_return_code PIDX_get_dims(PIDX_file file, PIDX_point dims)
{
  ...
  memcpy(dims, file->idx_ptr->global_bounds, (sizeof(int64) * PIDX_MAX_DIMENSIONS));
...
PIDX_return_code PIDX_read_next_variable(PIDX_variable variable, PIDX_point offset, PIDX_point dims, void* write_to_this_buffer, PIDX_data_layout data_layout)
{
  ...
  memcpy(variable->patch[variable->patch_count]->Ndim_box_offset, offset, PIDX_MAX_DIMENSIONS * sizeof(int64));
  memcpy(variable->patch[variable->patch_count]->Ndim_box_size, dims, PIDX_MAX_DIMENSIONS * sizeof(int64));
...
PIDX_return_code populate_idx_dataset(PIDX_file file)
{
  ...
  file->idx_derived_ptr->max_file_count = (getPowerOf2(file->idx_ptr->global_bounds[0]) * getPowerOf2(file->idx_ptr->global_bounds[1]) * getPowerOf2(file->idx_ptr->global_bounds[2]) * getPowerOf2(file->idx_ptr->global_bounds[3]) * getPowerOf2(file->idx_ptr->global_bounds[4])) / ((uint64) file->idx_derived_ptr->samples_per_block * (uint64) file->idx_ptr->blocks_per_file);
  if ((getPowerOf2(file->idx_ptr->global_bounds[0]) * getPowerOf2(file->idx_ptr->global_bounds[1]) * getPowerOf2(file->idx_ptr->global_bounds[2]) * getPowerOf2(file->idx_ptr->global_bounds[3]) * getPowerOf2(file->idx_ptr->global_bounds[4])) % ((uint64) file->idx_derived_ptr->samples_per_block * (uint64) file->idx_ptr->blocks_per_file))
...
PIDX_return_code PIDX_read(PIDX_file file)
{
      ...
      start_index = start_index + (file->variable_pipelining_factor + 1))
  {
	...
	for (p = 0; p < file->idx_ptr->variable[var]->patch_group_count; p++)
	{
	  ...
	  for(j = 0; j < file->idx_ptr->variable[var]->patch_group_ptr[p]->box_count; j++)
	  {
	    ...
	    memcpy(file->idx_ptr->variable[var]->patch_group_ptr[p]->box[j]->Ndim_box_offset, file->idx_ptr->variable[var]->patch[p]->Ndim_box_offset, PIDX_MAX_DIMENSIONS * sizeof(int));
	    memcpy(file->idx_ptr->variable[var]->patch_group_ptr[p]->box[j]->Ndim_box_size, file->idx_ptr->variable[var]->patch[p]->Ndim_box_size, PIDX_MAX_DIMENSIONS * sizeof(int));
    ...
    if(do_agg == 1)
    {
      ...
      PIDX_io_aggregated_IO(file->io_id, file->idx_derived_ptr->agg_buffer, PIDX_READ);
      PIDX_agg_aggregate_write_read(file->agg_id, file->idx_derived_ptr->agg_buffer, PIDX_READ);
      ...
      start_index = start_index + (file->variable_pipelining_factor + 1))
  {
	...
	for(j = 0; j < file->idx_ptr->variable[var]->patch_group_ptr[p]->count; j++)
	{
	  ...
	  memcpy(file->idx_ptr->variable[var]->patch_group_ptr[p]->block[j]->offset, file->idx_ptr->variable[var]->patch[p]->offset, PIDX_MAX_DIMENSIONS * sizeof(int64));
	  memcpy(file->idx_ptr->variable[var]->patch_group_ptr[p]->block[j]->count, file->idx_ptr->variable[var]->patch[p]->count, PIDX_MAX_DIMENSIONS * sizeof(int64));
    ...
    if(do_agg == 1)
    {
      ...
      PIDX_agg_buf_destroy(file->idx_derived_ptr->agg_buffer);
    }
    else
      PIDX_io_independent_IO_var(file->io_id, file->idx_ptr->variable, PIDX_READ);
  ...
  for (i = 0; i < file->idx_ptr->blocks_per_file; i++)
  {
    ...
    if (is_block_present((i + (file->idx_ptr->blocks_per_file * file->idx_derived_ptr->agg_buffer->file_number)), file->idx_ptr->variable[file->local_variable_index]->VAR_global_block_layout))
    {
      ...
      for (j = 0; j < file->idx_ptr->variable_count; j++)
      {
        ...
        {
          for (k = 0; k < j; k++)
            base_offset = base_offset + (file->idx_ptr->variable[file->local_variable_index]->VAR_blocks_per_file[file->idx_derived_ptr->agg_buffer->file_number] /*- empty_blocks*/) * (file->idx_ptr->variable[k]->bits_per_value / 8) * file->idx_derived_ptr->samples_per_block * file->idx_ptr->variable[k]->values_per_sample;
        }
        else
          base_offset =  j * (file->idx_ptr->variable[file->local_variable_index]->VAR_blocks_per_file[file->idx_derived_ptr->agg_buffer->file_number] /*- empty_blocks*/) * (file->idx_ptr->variable[file->local_variable_index]->bits_per_value / 8) * file->idx_derived_ptr->samples_per_block * file->idx_ptr->variable[file->local_variable_index]->values_per_sample;
  ...
  for (i = 0; i < file->idx_ptr->blocks_per_file; i++)
  {
    ...
    if (is_block_present((i + (file->idx_ptr->blocks_per_file * file->idx_derived_ptr->agg_buffer->file_number)), file->idx_derived_ptr->global_block_layout))
    {
      ...
      for (j = 0; j < file->idx_ptr->variable_count; j++)
      {
        ...
        {
          for (k = 0; k < j; k++)
            base_offset = base_offset + (file->idx_derived_ptr->existing_blocks_index_per_file[file->idx_derived_ptr->agg_buffer->file_number] /*- empty_blocks*/) * (file->idx_ptr->variable[k]->bits_per_value / 8) * file->idx_derived_ptr->samples_per_block * file->idx_ptr->variable[k]->values_per_sample;
        }
        else
          base_offset =  j * (file->idx_derived_ptr->existing_blocks_index_per_file[file->idx_derived_ptr->agg_buffer->file_number] /*- empty_blocks*/) * (file->idx_ptr->variable[file->local_variable_index]->bits_per_value / 8) * file->idx_derived_ptr->samples_per_block * file->idx_ptr->variable[file->local_variable_index]->values_per_sample;
...
static PIDX_return_code PIDX_write(PIDX_file file)
{
    ...
    if(global_do_rst == 0)
    {
      for (var = start_index; var <= end_index; var++)
      {
        for (p = 0; p < file->idx_ptr->variable[var]->patch_group_count; p++)
        {
          ...
          {
            file->idx_ptr->variable[var]->patch_group_ptr[p]->box[j] = malloc(sizeof(*(file->idx_ptr->variable[var]->patch_group_ptr[p]->box[j])));
            memcpy(file->idx_ptr->variable[var]->patch_group_ptr[p]->box[j]->Ndim_box_offset, file->idx_ptr->variable[var]->patch[p]->Ndim_box_offset, PIDX_MAX_DIMENSIONS * sizeof(int64));
            memcpy(file->idx_ptr->variable[var]->patch_group_ptr[p]->box[j]->Ndim_box_size, file->idx_ptr->variable[var]->patch[p]->Ndim_box_size, PIDX_MAX_DIMENSIONS * sizeof(int64));
            
            memcpy(file->idx_ptr->variable[var]->patch_group_ptr[p]->enclosing_box_offset, file->idx_ptr->variable[var]->patch[p]->Ndim_box_offset, PIDX_MAX_DIMENSIONS * sizeof(int64));
            memcpy(file->idx_ptr->variable[var]->patch_group_ptr[p]->enclosing_box_size, file->idx_ptr->variable[var]->patch[p]->Ndim_box_size, PIDX_MAX_DIMENSIONS * sizeof(int64));
            file->idx_ptr->variable[var]->patch_group_ptr[p]->box[j]->Ndim_box_buffer = file->idx_ptr->variable[var]->patch[p]->Ndim_box_buffer;
          }
        }
      }
      ...
      start_index = start_index + (file->variable_pipelining_factor + 1))
  {
	...
	file->idx_ptr->variable[var]->patch_group_ptr[p]->box = malloc(sizeof(*(file->idx_ptr->variable[var]->patch_group_ptr[p]->box)) * file->idx_ptr->variable[var]->patch_group_ptr[p]->count);
	for(j = 0; j < file->idx_ptr->variable[var]->patch_group_ptr[p]->count; j++)
	{
	  file->idx_ptr->variable[var]->patch_group_ptr[p]->box[j] = malloc(sizeof(*(file->idx_ptr->variable[var]->patch_group_ptr[p]->box[j])));
	  memcpy(file->idx_ptr->variable[var]->patch_group_ptr[p]->box[j]->offset, file->idx_ptr->variable[var]->patch[p]->offset, PIDX_MAX_DIMENSIONS * sizeof(int));
	  memcpy(file->idx_ptr->variable[var]->patch_group_ptr[p]->box[j]->count, file->idx_ptr->variable[var]->patch[p]->count, PIDX_MAX_DIMENSIONS * sizeof(int));
	  file->idx_ptr->variable[var]->patch_group_ptr[p]->box[j]->buffer = file->idx_ptr->variable[var]->patch[p]->buffer;
	}
      }
    }
...
{ 
  ...
  if (file->idx_count[0] *  file->idx_count[1] * file->idx_count[2] == 1)
  {
    if (max_time == total_time)
    {
      ...
      int64 total_data = file->idx_ptr->global_bounds[0] * file->idx_ptr->global_bounds[1] * file->idx_ptr->global_bounds[2] * file->idx_ptr->global_bounds[3] * file->idx_ptr->global_bounds[4] * sample_sum * 8;
      fprintf(stdout, "\n=======================================================================================\n");
  ...
  else
  {
    ...
    int64 total_data = file->idx_ptr->global_bounds[0] * file->idx_ptr->global_bounds[1] * file->idx_ptr->global_bounds[2] * file->idx_ptr->global_bounds[3] * file->idx_ptr->global_bounds[4] * sample_sum * 8 * file->idx_count[0] * file->idx_count[1] * file->idx_count[2];
    if (max_time == total_time)
...
#else
  memcpy(rank_r_offset, variable->patch[0]->Ndim_box_offset, PIDX_MAX_DIMENSIONS * sizeof(int64));
  memcpy(rank_r_count, variable->patch[0]->Ndim_box_size, PIDX_MAX_DIMENSIONS * sizeof(int64));
#endif
