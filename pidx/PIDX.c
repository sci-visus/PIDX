...
PIDX_return_code PIDX_file_create(const char* filename, PIDX_flags flags, PIDX_access access_type, PIDX_file* file)
{
  ...
  *file = malloc(sizeof (*(*file)) );
  ...
  if (access_type->parallel)
  {
    ...
    if (access_type->topology_aware_io == 0)
    {
      if ((*file)->idx_count[0] != 1 || (*file)->idx_count[1] != 1 || (*file)->idx_count[2] != 1 )
      {
        ...
        {
          rank_x |= ((morton & (uint64_t( 1ull ) << uint64_t((3ull * i) + 0ull))) >> uint64_t(((3ull * i) + 0ull)-i));
          rank_y |= ((morton & (uint64_t( 1ull ) << uint64_t((3ull * i) + 1ull))) >> uint64_t(((3ull * i) + 1ull)-i));
          rank_z |= ((morton & (uint64_t( 1ull ) << uint64_t((3ull * i) + 2ull))) >> uint64_t(((3ull * i) + 2ull)-i));
        }
        
        morton = rank;
        for (i = 0; i < (sizeof(uint64_t) * CHAR_BIT)/2; ++i)
        {
          rank_x |= ((morton & (uint64_t( 1ull ) << uint64_t((2ull * i) + 0ull))) >> uint64_t(((2ull * i) + 0ull)-i));
          //rank_y |= ((morton & (uint64_t( 1ull ) << uint64_t((2ull * i) + 1ull))) >> uint64_t(((2ull * i) + 1ull)-i));
          //rank_z |= ((morton & (uint64_t( 1ull ) << uint64_t((3ull * i) + 2ull))) >> uint64_t(((3ull * i) + 2ull)-i));
        }
        */
    ...
    else
    {
      ...
      int iotask = 1;
      (*file)->idx_derived_ptr->color = identity(access_type->comm, access_type->comm, &iotask);
      //(*file)->idx_derived_ptr->color = (*file)->idx_derived_ptr->color/2;
#else
       srand(time(NULL) * rank);
       (*file)->idx_derived_ptr->color = /*rank % 2;*/ rand() % 8;
       printf("Rank %d has color %d\n", rank, (*file)->idx_derived_ptr->color);
#endif
      MPI_Comm_split(access_type->comm, (*file)->idx_derived_ptr->color, rank, &((*file)->comm));
      MPI_Comm_dup(access_type->comm, &((*file)->global_comm));
    }
  ...
  (*file)->idx_ptr->compression_bit_rate = 64;
...
PIDX_return_code PIDX_get_lossy_compression_bit_rate(PIDX_file file, int *compression_bit_rate)
{
  ...
  *compression_bit_rate = file->idx_ptr->compression_bit_rate;
...
PIDX_return_code populate_idx_dataset(PIDX_file file)
{
  ...
  if (file->access->topology_aware_io == 1)
  {
    for (i = 0; i < PIDX_MAX_DIMENSIONS; i++) 
    {
      bounding_box[0][i] = 0;
  ...
  else
  {
    ...
    PIDX_blocks_initialize_layout(all_patch_local_block_layout, file->idx_derived_ptr->maxh, file->idx_ptr->bits_per_block);
    
    file->idx_derived_ptr->global_block_layout = malloc(sizeof (*file->idx_derived_ptr->global_block_layout));
    PIDX_blocks_initialize_layout(file->idx_derived_ptr->global_block_layout, file->idx_derived_ptr->maxh, file->idx_ptr->bits_per_block);
    
    for (p = 0 ; p < file->idx_ptr->variable[file->local_variable_index]->patch_count ; p++)
    {
      for (i = 0; i < PIDX_MAX_DIMENSIONS; i++)
      {
        bounding_box[0][i] = file->idx_ptr->variable[file->local_variable_index]->patch[p]->Ndim_box_offset[i];
        bounding_box[1][i] = file->idx_ptr->variable[file->local_variable_index]->patch[p]->Ndim_box_size[i] + file->idx_ptr->variable[file->local_variable_index]->patch[p]->Ndim_box_offset[i];
      }
    ...
    {
#if PIDX_HAVE_MPI
      MPI_Allreduce(all_patch_local_block_layout->hz_block_number_array[i], file->idx_derived_ptr->global_block_layout->hz_block_number_array[i], level_count,
                MPI_INT, MPI_BOR, file->comm);
#else
      memcpy(file->idx_derived_ptr->global_block_layout->hz_block_number_array[i], all_patch_local_block_layout->hz_block_number_array[i], level_count * sizeof(int));
#endif
      level_count = level_count * 2;
    }
    ...
    for (i = 1; i < (file->idx_derived_ptr->global_block_layout->levels); i++)
    {
      ...
      {
        if(file->idx_derived_ptr->global_block_layout->hz_block_number_array[i][j] != 0)
          file->idx_derived_ptr->global_block_layout->hz_block_count_array[i]++;
      }    
...
static PIDX_return_code PIDX_cache_headers(PIDX_file file)
{
  ...
  int64_t total_compression_block_size = (file->idx_ptr->compression_block_size[0] * file->idx_ptr->compression_block_size[1] * file->idx_ptr->compression_block_size[2] * file->idx_ptr->compression_block_size[3] * file->idx_ptr->compression_block_size[4]) / (64 / file->idx_ptr->compression_bit_rate);
  ...
  for (i = 0; i < file->idx_ptr->blocks_per_file; i++)
  {
    ...
    if (PIDX_blocks_is_block_present((i + (file->idx_ptr->blocks_per_file * file->idx_derived_ptr->agg_buffer->file_number)), file->idx_derived_ptr->global_block_layout))
    {
      ...
      for (j = 0; j < file->idx_ptr->variable_count; j++)
      {
        ...
        //printf("[%d %d] O C %lld %lld\n", i, j, (long long)data_offset, (long long)file->idx_derived_ptr->samples_per_block * (file->idx_ptr->variable[j]->bits_per_value / 8) * total_compression_block_size * file->idx_ptr->variable[j]->values_per_sample);
        cached_header_copy[12 + ((i + (file->idx_ptr->blocks_per_file * j))*10)] = htonl(data_offset);
        cached_header_copy[14 + ((i + (file->idx_ptr->blocks_per_file * j))*10)] = htonl(file->idx_derived_ptr->samples_per_block * (file->idx_ptr->variable[j]->bits_per_value / 8) * total_compression_block_size * file->idx_ptr->variable[j]->values_per_sample);  
      }
      //printf("\n");
  ...
  for (start_index = file->local_variable_index; start_index < file->local_variable_index + file->local_variable_count; start_index = start_index + (file->variable_pipelining_factor + 1))
  {
    ...
    if(do_agg == 1)
    {
      ...
      */
      ...
      {
        if (file->idx_ptr->enable_compression == 0 || (file->idx_ptr->enable_compression == 1 && file->idx_ptr->compression_type == 1))
        {
