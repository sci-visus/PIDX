...
int main(int argc, char **argv)
{
  ...
  for (ts = 0; ts < time_step_count; ts++)
  {
    ...
    for (var = 0; var < variable_count; var++)
    {
      ...
      data = malloc(sizeof (uint64_t) * local_box_size[0] * local_box_size[1] * local_box_size[2]  * values_per_sample);
        ...
        for (j = 0; j < local_box_size[1]; j++)
          for (i = 0; i < local_box_size[0]; i++)
          {
            ...
            for (vps = 0; vps < values_per_sample; vps++)
              data[index * values_per_sample + vps] = 100 + var + vps + ((global_box_size[0] * global_box_size[1]*(local_box_offset[2] + k))+(global_box_size[0]*(local_box_offset[1] + j)) + (local_box_offset[0] + i));
          }

      ret = PIDX_variable_create(var_name, values_per_sample * sizeof(uint64_t) * 8, type, &variable);
      ...
      ret = PIDX_flush(file);
      if (ret != PIDX_success)  report_error("PIDX_flush", __FILE__, __LINE__);
