...
static PIDX_return_code populate_idx_layout(PIDX_block_layout block_layout)
{
  int i, j;
  int ctr = 1;
  ...
  {
    bounding_box[0][i] = 0;
    bounding_box[1][i] = chunked_bounds[i];
  }

  ret_code = PIDX_blocks_create_layout (bounding_box, maxh, bitPattern, block_layout, 0/*reduced_res_from*/, 0/*reduced_res_to*/);
  ...
  block_layout->file_bitmap = malloc(max_file_count * sizeof (int));
  memset(block_layout->file_bitmap, 0, max_file_count * sizeof (int));

  block_layout->file_index = malloc(sizeof(int) * (max_file_count));
  memset(block_layout->file_index, 0, sizeof(int) * (max_file_count));

  block_layout->block_count_per_file = malloc(sizeof(int) * (max_file_count));
  memset(block_layout->block_count_per_file, 0, sizeof(int) * (max_file_count));


  int file_number = 0;
  if (block_layout->resolution_from <= block_layout->bits_per_block)
  {
    for (i = block_layout->resolution_from ; i <= bits_per_block ; i++)
    {
      if (block_layout->hz_block_number_array[i][0] == 0)
      {
        file_number = block_layout->hz_block_number_array[i][0] / blocks_per_file;
    ...
    for (i = block_layout->bits_per_block + 1 ; i < block_layout->resolution_to ; i++)
    {
      for (j = 0; j < ctr; j++)
      {
        ...
        {
          file_number = block_layout->hz_block_number_array[i][j] / blocks_per_file;
          block_layout->file_bitmap[file_number] = 1;
          block_layout->file_index[file_number] = 1;
          block_layout->block_count_per_file[file_number]++;
        }
  ...
  block_layout->inverse_existing_file_index = (int*) malloc(max_file_count * sizeof (int));
  memset(block_layout->inverse_existing_file_index, 0, max_file_count * sizeof (int));
...
static PIDX_return_code populate_idx_dataset_file_zero(int hz_level_from, int hz_level_to)
{
  int i = 0, j = 0, ctr;
  int file_number = 0;
  ...
  {
    start_layout_index_file_zero = 0;
    end_layout_index_file_zero = 0;

    layout_count_file_zero = 0;
    return PIDX_success;
  }
  ...
  global_block_layout_file_zero = malloc(sizeof (*(global_block_layout_file_zero)));
  memset(global_block_layout_file_zero, 0, sizeof (*(global_block_layout_file_zero)));
  PIDX_block_layout block_layout = global_block_layout_file_zero;

  lower_hz_level = hz_level_from;
  higher_hz_level = hz_level_to;
  ret_code = PIDX_blocks_initialize_layout(block_layout, lower_hz_level, higher_hz_level, maxh, bits_per_block);
  if (ret_code != PIDX_success)
  {
...
#if 1
  start_layout_index_file_zero = (lower_hz_level - (bits_per_block + log2(blocks_per_file)));
  if (start_layout_index_file_zero <= 0)
    start_layout_index_file_zero = 0;
#else
  end_layout_index_file_zero = 0;
#endif
...
#if 1
  end_layout_index_file_zero = (higher_hz_level - (bits_per_block + log2(blocks_per_file)));
  if (end_layout_index_file_zero <= 0)
    end_layout_index_file_zero = 1;
#else
  end_layout_index_file_zero = 1;
#endif


  layout_count_file_zero = end_layout_index_file_zero - start_layout_index_file_zero;

  local_block_layout_file_zero = malloc(sizeof(*(local_block_layout_file_zero)) * layout_count_file_zero);
  memset(local_block_layout_file_zero, 0, sizeof(*(local_block_layout_file_zero)) * layout_count_file_zero);

  int i_1 = 0;
  for (i = start_layout_index_file_zero; i < end_layout_index_file_zero ; i++)
  {
    i_1 = i - start_layout_index_file_zero;
    local_block_layout_file_zero[i_1] = malloc(sizeof(*(local_block_layout_file_zero[i_1])));
    memset(local_block_layout_file_zero[i_1], 0, sizeof(*(local_block_layout_file_zero[i_1])));
  }

  if (start_layout_index_file_zero == 0)
  {
    lower_level_low_layout = 0;
    higher_level_low_layout = bits_per_block + log2(blocks_per_file) + 1;

    if (higher_level_low_layout >= higher_hz_level)
      higher_level_low_layout = higher_hz_level;

    ret_code = PIDX_blocks_initialize_layout(local_block_layout_file_zero[0], lower_level_low_layout, higher_level_low_layout, maxh, bits_per_block);
    if (ret_code != PIDX_success)
    {
      fprintf(stderr, "[%s] [%d ]Error in PIDX_blocks_initialize_layout", __FILE__, __LINE__);
      return PIDX_err_file;
    ...
    ret_code = populate_idx_layout(local_block_layout_file_zero[0]);
    if (ret_code != PIDX_success)
    {
    ...
    for (j = lower_hz_level ; j < bits_per_block + 1 ; j++)
      memcpy(block_layout->hz_block_number_array[j], local_block_layout_file_zero[0]->hz_block_number_array[j], sizeof(int));

    ctr = 1;
    int temp_level = bits_per_block + log2(blocks_per_file) + 1;
    if (temp_level >= higher_hz_level)
      temp_level = higher_hz_level;
    for (j = bits_per_block + 1 ; j < temp_level ; j++)
    {
      memcpy(block_layout->hz_block_number_array[j], local_block_layout_file_zero[0]->hz_block_number_array[j], sizeof(int) * ctr);
      ctr = ctr * 2;
    }

    for (i = 1; i < layout_count_file_zero; i++)
    {
      lower_level_higher_layout = bits_per_block + log2(blocks_per_file) + 1 + (i - 1);
      higher_level_higher_layout = lower_level_higher_layout + 1;

      ret_code = PIDX_blocks_initialize_layout(local_block_layout_file_zero[i], lower_level_higher_layout, higher_level_higher_layout, maxh, bits_per_block);
      if (ret_code != PIDX_success)
      {
        fprintf(stderr, "[%s] [%d ]Error in PIDX_blocks_initialize_layout", __FILE__, __LINE__);
        return PIDX_err_file;
      }
      ret_code = populate_idx_layout(local_block_layout_file_zero[i]);
      if (ret_code != PIDX_success)
      {
        fprintf(stderr, "[%s] [%d ]Error in populate_idx_layout\n", __FILE__, __LINE__);
        return PIDX_err_file;
      ...
      memcpy(block_layout->hz_block_number_array[lower_level_higher_layout], local_block_layout_file_zero[i]->hz_block_number_array[lower_level_higher_layout], sizeof(int) * ctr);
      ctr = ctr * 2;
    }
  }
  else
  {
    ...
    ctr = (int)pow(2, start_layout_index_file_zero - 1) * blocks_per_file;
    for (i = start_layout_index_file_zero; i < end_layout_index_file_zero; i++)
    {
      lower_level_higher_layout = bits_per_block + log2(blocks_per_file) + 1 + (i - 1);
      higher_level_higher_layout = lower_level_higher_layout + 1;

      ret_code = PIDX_blocks_initialize_layout(local_block_layout_file_zero[i - start_layout_index_file_zero], lower_level_higher_layout, higher_level_higher_layout, maxh, bits_per_block);
      if (ret_code != PIDX_success)
      {
        fprintf(stderr, "[%s] [%d ]Error in PIDX_blocks_initialize_layout", __FILE__, __LINE__);
        return PIDX_err_file;
      }
      ret_code = populate_idx_layout(local_block_layout_file_zero[i - start_layout_index_file_zero]);
      if (ret_code != PIDX_success)
      {
        fprintf(stderr, "[%s] [%d ]Error in populate_idx_layout\n", __FILE__, __LINE__);
        return PIDX_err_file;
      ...
      memcpy(block_layout->hz_block_number_array[lower_level_higher_layout], local_block_layout_file_zero[i - start_layout_index_file_zero]->hz_block_number_array[lower_level_higher_layout], sizeof(int) * ctr);
      ctr = ctr * 2;
    }
  }
...
static PIDX_return_code populate_idx_dataset_shared(int hz_level_from, int hz_level_to)
{
  int i = 0, j = 0, ctr;
  int file_number = 0;
  ...
  if (hz_level_from == hz_level_to)
  {
    start_layout_index_shared = 0;
    end_layout_index_shared = 0;
    layout_count_shared = 0;
    return PIDX_success;
  }
  ...
  global_block_layout_shared_files = malloc(sizeof (*global_block_layout_shared_files));
  memset(global_block_layout_shared_files, 0, sizeof (*global_block_layout_shared_files));
  PIDX_block_layout block_layout = global_block_layout_shared_files;
  ...
  start_layout_index_shared = (lower_hz_level - (bits_per_block + log2(blocks_per_file)));
  if (start_layout_index_shared <= 0)
    start_layout_index_shared = 0;

  end_layout_index_shared = (higher_hz_level - (bits_per_block + log2(blocks_per_file)));
  if (end_layout_index_shared <= 0)
    end_layout_index_shared = 1;

  layout_count_shared = end_layout_index_shared - start_layout_index_shared;

  local_block_layout_shared_files = malloc(sizeof(*(local_block_layout_shared_files)) * layout_count_shared);
  memset(local_block_layout_shared_files, 0, sizeof(*(local_block_layout_shared_files)) * layout_count_shared);
  //for (i = 0; i < layout_count ; i++)
  for (i = start_layout_index_shared; i < end_layout_index_shared ; i++)
  {
    local_block_layout_shared_files[i - start_layout_index_shared] = malloc(sizeof(*(local_block_layout_shared_files[i - start_layout_index_shared])));
    memset(local_block_layout_shared_files[i - start_layout_index_shared], 0, sizeof(*(local_block_layout_shared_files[i - start_layout_index_shared])));
  }
  ...
  if (start_layout_index_shared == 0)
  {
    ...
    ret_code = PIDX_blocks_initialize_layout(local_block_layout_shared_files[0], lower_level_low_layout, higher_level_low_layout, maxh, bits_per_block);
    if (ret_code != PIDX_success)
    {
      fprintf(stderr, "[%s] [%d ]Error in PIDX_blocks_initialize_layout", __FILE__, __LINE__);
      return PIDX_err_file;
    ...
    ret_code = populate_idx_layout(local_block_layout_shared_files[0]);
    if (ret_code != PIDX_success)
    {
      fprintf(stderr, "[%s] [%d ]Error in populate_idx_layout\n", __FILE__, __LINE__);
      return PIDX_err_file;
    ...
    for (j = lower_hz_level ; j < bits_per_block + 1 ; j++)
      memcpy(block_layout->hz_block_number_array[j], local_block_layout_shared_files[0]->hz_block_number_array[j], sizeof(int));
    ...
    for (j = bits_per_block + 1 ; j < temp_level ; j++)
    {
      memcpy(block_layout->hz_block_number_array[j], local_block_layout_shared_files[0]->hz_block_number_array[j], sizeof(int) * ctr);
      ctr = ctr * 2;
    }

    for (i = 1; i < layout_count_shared; i++)
    {
      lower_level_higher_layout = bits_per_block + log2(blocks_per_file) + 1 + (i - 1);
      higher_level_higher_layout = lower_level_higher_layout + 1;

      ret_code = PIDX_blocks_initialize_layout(local_block_layout_shared_files[i], lower_level_higher_layout, higher_level_higher_layout, maxh, bits_per_block);
      if (ret_code != PIDX_success)
      {
        fprintf(stderr, "[%s] [%d ]Error in PIDX_blocks_initialize_layout", __FILE__, __LINE__);
        return PIDX_err_file;
      }
      ret_code = populate_idx_layout(local_block_layout_shared_files[i]);
      if (ret_code != PIDX_success)
      {
        fprintf(stderr, "[%s] [%d ]Error in populate_idx_layout\n", __FILE__, __LINE__);
        return PIDX_err_file;
      ...
      memcpy(block_layout->hz_block_number_array[lower_level_higher_layout], local_block_layout_shared_files[i]->hz_block_number_array[lower_level_higher_layout], sizeof(int) * ctr);
      ctr = ctr * 2;
    }
    local_block_layout_shared_files[0]->resolution_from = hz_level_from;
  }
  else
  {
    ctr = 1;
    ...
    ctr = (int)pow(2, start_layout_index_shared - 1) * blocks_per_file;
    for (i = start_layout_index_shared; i < end_layout_index_shared; i++)
    {
      lower_level_higher_layout = bits_per_block + log2(blocks_per_file) + 1 + (i - 1);
      higher_level_higher_layout = lower_level_higher_layout + 1;

      ret_code = PIDX_blocks_initialize_layout(local_block_layout_shared_files[i - start_layout_index_shared], lower_level_higher_layout, higher_level_higher_layout, maxh, bits_per_block);
      if (ret_code != PIDX_success)
      {
        fprintf(stderr, "[%s] [%d ]Error in PIDX_blocks_initialize_layout", __FILE__, __LINE__);
        return PIDX_err_file;
      }
      ret_code = populate_idx_layout(local_block_layout_shared_files[i - start_layout_index_shared]);
      if (ret_code != PIDX_success)
      {
        fprintf(stderr, "[%s] [%d ]Error in populate_idx_layout\n", __FILE__, __LINE__);
        return PIDX_err_file;
      ...
      memcpy(block_layout->hz_block_number_array[lower_level_higher_layout], local_block_layout_shared_files[i - start_layout_index_shared]->hz_block_number_array[lower_level_higher_layout], sizeof(int) * ctr);
      ctr = ctr * 2;
    }
  }
  ...
  //  PIDX_blocks_print_layout(/*block_layout*/local_block_layout_shared_files);
  //}
  //
...
int main(int argc, char **argv)
{
  ...
  int hz_to_file_zero =  shared_block_level;
  ret = populate_idx_dataset_file_zero(hz_from_file_zero, hz_to_file_zero);
  if (ret != PIDX_success)
  {
  ...
  int hz_from_shared = shared_block_level;
  int hz_to_shared =  total_partiton_level;
  ret = populate_idx_dataset_shared(hz_from_shared, hz_to_shared);
  if (ret != PIDX_success)
  {
    fprintf(stdout,"File %s Line %d\n", __FILE__, __LINE__);
    return PIDX_err_file;
  ...
  printf("Shared Block Level %d total partition level %d\n", shared_block_level, total_partiton_level);
  int level = 0;
  int ts = 0;
