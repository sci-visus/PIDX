...
/* compress 3d strided array */
static void
_t2(compress_strided2, Scalar, 3)(zfp_stream* stream, const zfp_field* field)
{
  const Scalar* data = field->data;
  uint nx = field->nx;
  uint ny = field->ny;
  ...
  for (z = 0; z < mz; z += 4, data += 4 * sz - my * sy) {
    for (y = 0; y < my; y += 4, data += 4 * sy - mx * sx) {
      for (x = 0; x < mx; x += 4, data += 4 * sx)
        _t2(zfp_encode_block_strided2, Scalar, 3)(stream, data, sx, sy, sz);
      if (x < nx)
        _t2(zfp_encode_partial_block_strided2, Scalar, 3)(stream, data, nx - x, 4, 4, sx, sy, sz);
    }
    if (y < ny) {
      for (x = 0; x < mx; x += 4, data += 4 * sx)
        _t2(zfp_encode_partial_block_strided2, Scalar, 3)(stream, data, 4, ny - y, 4, sx, sy, sz);
      if (x < nx)
        _t2(zfp_encode_partial_block_strided2, Scalar, 3)(stream, data, nx - x, ny - y, 4, sx, sy, sz);
      data -= mx * sx;
    }
  }
  if (z < nz) {
    for (y = 0; y < my; y += 4, data += 4 * sy - mx * sx) {
      for (x = 0; x < mx; x += 4, data += 4 * sx)
        _t2(zfp_encode_partial_block_strided2, Scalar, 3)(stream, data, 4, 4, nz - z, sx, sy, sz);
      if (x < nx)
        _t2(zfp_encode_partial_block_strided2, Scalar, 3)(stream, data, nx - x, 4, nz - z, sx, sy, sz);
    }
    if (y < ny) {
      for (x = 0; x < mx; x += 4, data += 4 * sx)
        _t2(zfp_encode_partial_block_strided2, Scalar, 3)(stream, data, 4, ny - y, nz - z, sx, sy, sz);
      if (x < nx)
        _t2(zfp_encode_partial_block_strided2, Scalar, 3)(stream, data, nx - x, ny - y, nz - z, sx, sy, sz);
    }
  }
}
